...
const retryFailedWebClipImagesBtn = $("retryFailedWebClipImagesBtn");
const webClipExistingNoteSection = $("webClipExistingNoteSection");
const webClipExistingNoteSummary = $("webClipExistingNoteSummary");
const webClipExistingNoteSelectRow = $("webClipExistingNoteSelectRow");
const webClipExistingNoteSelect = $("webClipExistingNoteSelect");
const webClipSaveAsUpdate = $("webClipSaveAsUpdate");
...
const localSaveEnabled = $("localSaveEnabled");
...
const localFolderDetails = $("localFolderDetails");
const localFolderStatus = $("localFolderStatus");
const localScanResults = $("localScanResults");
...
function setLocalSaveState(status, patch = {}) {
  ...
  if (db) void localFs.putConfig(db, LOCAL_CONFIG_STORE_NAME, "state", localSaveState).catch((error) => console.warn("Local save state persistence failed", error));
  renderSaveStatus();
...
async function initializeLocalFolderSaving() {
  ...
  if (!support.supported) {
    setLocalSaveState("unsupported", { errorCode: "unsupported", errorMessage: support.reason, requiresUserAction: false });
    return;
  }
  if (!localDirectoryHandle || !localSaveSettings.enabled) {
    setLocalSaveState("unconfigured", { directoryName: localDirectoryHandle?.name || "", pendingChanges: false, requiresUserAction: false });
    return;
  }
...
function queueLocalWorkspaceSave(reason = "change") {
  if (suppressLocalSaveQueue || !localSaveSettings.enabled || !localDirectoryHandle) return Promise.resolve(false);
  const support = localSupport();
  if (!support.supported) {
    setLocalSaveState("unsupported", { errorCode: "unsupported", errorMessage: support.reason, requiresUserAction: false });
  ...
  const queued = localSaveQueue.enqueue(reason);
  queued.catch((error) => console.warn("Local workspace save failed", error));
  return queued;
}
  ...
  try {
    ...
    for (const note of storedNotes) {
      ...
      const attachmentFiles = noteAttachments.map((attachment) => {
        ...
        assetPlans.set(attachment.id, { ...attachment, fileName: assetFileName });
        nextSync.assets[attachment.id] = { fileName: assetFileName, memoId: note.id, mimeType: attachment.mimeType || attachment.blob?.type || "" };
        return { id: attachment.id, fileName: assetFileName };
      });
...
async function selectLocalSaveFolder() {
  const support = localSupport();
  if (!support.supported) {
    setLocalSaveState("unsupported", { errorCode: "unsupported", errorMessage: support.reason, requiresUserAction: false });
    return;
...
async function setLocalSaveEnabled(enabled) {
  localSaveSettings = { enabled: Boolean(enabled) };
  await persistLocalSaveSettings();
  if (!enabled) {
    setLocalSaveState("unconfigured", { directoryName: localDirectoryHandle?.name || "", pendingChanges: false, requiresUserAction: false });
    return;
  ...
  if (!localDirectoryHandle) {
    setLocalSaveState("unconfigured", { errorMessage: "フォルダを選択してください。", directoryName: "" });
    return;
  }
  const permission = await localFs.queryPermission(localDirectoryHandle, "readwrite");
  if (permission !== "granted") {
...
function noteFromLocalCandidate(candidate, { preserveId = false, overwrite = null } = {}) {
  const metadata = candidate.parsed.metadata;
  const fallbackCreatedAt = candidate.parsed.displayedCreatedAt;
  return {
    ...(overwrite || {}),
    id: preserveId && metadata.memoNexusId ? String(metadata.memoNexusId) : overwrite?.id || crypto.randomUUID(),
    title: String(metadata.title || candidate.name.replace(/\.md$/i, "") || "外部Markdown"),
    body: candidate.parsed.body,
    collectionId: collectionExists(metadata.collectionId) ? metadata.collectionId : UNCLASSIFIED_COLLECTION_ID,
    createdAt: metadata.createdAt || metadata.localCreatedAt || fallbackCreatedAt,
    localCreatedAt: metadata.localCreatedAt || metadata.createdAt || fallbackCreatedAt,
    updatedAt: metadata.updatedAt || candidate.file.lastModified || Date.now(),
...
async function applyLocalCandidate(index, action) {
  ...
  if (!candidate) return;
  if (action === "hold") {
    setLocalSaveState("conflict", { directoryName: localDirectoryHandle?.name || "", errorCode: "conflict-held", errorMessage: "競合を保留しています。どちらを残すか選ぶまで自動上書きしません。", requiresUserAction: true });
    return;
  }
  if (action === "exclude") {
...
async function restoreCollectionsFromLocal() {
  const root = await localFs.resolveWorkspaceRoot(localDirectoryHandle, false);
  const text = await optionalLocalText(root, "collections.json");
  if (!text) return 0;
  const imported = parseCollections(text);
  const existingIds = new Set(collections.map((item) => item.id));
  const additions = imported.filter((item) => !existingIds.has(item.id));
  if (!additions.length) return 0;
  suppressLocalSaveQueue = true;
  try {
    for (const collection of additions) await putCollection(collection);
  } finally {
    suppressLocalSaveQueue = false;
  }
  collections = await getAllCollections();
...
async function restoreFromLocalFolder() {
  ...
  for (const candidate of candidates) {
    if (!["restore", "new"].includes(candidate.classification.type)) continue;
    const index = localScanCandidates.indexOf(candidate);
...
function renderLocalScanResults() {
  if (!localScanResults) return;
  localScanResults.innerHTML = "";
  localScanCandidates.forEach((candidate, index) => {
    const item = document.createElement("article");
    item.className = "local-scan-item";
    const typeLabels = { new: "新規候補", restore: "復元候補", conflict: "競合", duplicate: "重複候補" };
    const title = escapeHtml(candidate.parsed.metadata.title || candidate.name);
...
function renderLocalFolderSettings() {
  if (!localFolderDetails) return;
  const support = localSupport();
  const permissionLabel = localSaveState.status === "permission-required" ? "再接続が必要" : localDirectoryHandle ? (localSaveState.status === "unsupported" ? "非対応" : "接続済み") : "未接続";
  localSaveEnabled.checked = Boolean(localSaveSettings.enabled);
  localSaveEnabled.disabled = !support.supported;
  ...
  localFolderDetails.innerHTML = [
    ["選択中のフォルダ", localDirectoryHandle?.name || "未選択"],
    ["権限状態", permissionLabel],
...
function renderLocalSaveWarning() {
  if (!storageWarning) return;
  const warning = localSaveSettings.enabled && ["permission-required", "conflict", "error"].includes(localSaveState.status);
  if (warning) {
    storageWarning.dataset.localSaveWarning = "true";
    storageWarning.textContent = `ローカル保存: ${localSaveLabel(localSaveState.status)}。${localSaveState.errorMessage || "設定を確認してください。"}`;
    storageWarning.hidden = false;
    storageWarning.classList.toggle("storage-warning-strong", localSaveState.status === "error");
  } else if (storageWarning.dataset.localSaveWarning) {
    delete storageWarning.dataset.localSaveWarning;
    warnIfStorageRisky(checkLocalStorageAvailable(), notes.length);
  }
}
...
function webClipImageFailureReason(image) {
  const labels = {
    PERMISSION_DENIED: "画像ホストへのアクセス権限がありません",
    AUTH_REQUIRED: "画像を取得するにはログインが必要です",
    URL_EXPIRED: "画像URLの有効期限が切れている可能性があります",
    ACCESS_DENIED: "画像サーバーがアクセスを拒否しました",
    TIMEOUT: "画像取得がタイムアウトしました",
    NETWORK_ERROR: "ネットワーク経由で画像を取得できませんでした",
    TOO_LARGE: "画像が容量上限を超えています",
    TOTAL_LIMIT_EXCEEDED: "画像合計が容量上限を超えています",
    UNSUPPORTED_FORMAT: "対応していない画像形式です"
  };
  return image.saveError || labels[image.errorCode] || image.error || "画像を取得できませんでした";
}

function findMatchingWebClipNotes(url) {
  const normalized = normalizeWebClipComparisonUrl(url);
  if (!normalized) return [];
  return activeNotes().filter((note) => normalizeWebClipComparisonUrl(note.source?.url) === normalized);
}

function renderWebClipExistingNoteChoice() {
  if (!webClipExistingNoteSection) return;
  const matches = pendingWebClipMatches;
  webClipExistingNoteSection.hidden = !matches.length;
  if (!matches.length) return;
  webClipExistingNoteSummary.textContent = matches.length === 1
    ? `「${matches[0].title || "無題"}」を更新します。メモID、コレクション、フラグと追加メタデータは維持されます。`
    : `同じURLのWebクリップが${matches.length}件あります。更新対象を選ぶか、新しいメモとして保存してください。`;
...
function renderWebClipImages() {
  ...
  webClipImagesSummary.textContent = [
    ...
    failed.length ? `本文は保存できますが、画像${pendingWebClipImages.length}件のうち${failed.length}件を取得できませんでした。` : "",
  ...
  pendingWebClipImages.forEach((image, index) => {
    ...
    [
      ["状態", `${webClipImageStatusLabel(image)}${image.errorCode ? ` [${image.errorCode}]` : ""}${image.status === "ready" ? "" : `: ${webClipImageFailureReason(image)}`}`],
      ["説明", image.alt || "—"],
...
async function createWebClipNoteWithImages(title, clip, source, collectionId, existingNote = null) {
  const memoId = existingNote?.id || crypto.randomUUID();
  ...
  const previousWebClipAttachments = existingNote
    ? (await getAttachmentsForMemo(memoId)).filter((attachment) => attachment.source?.type === "web-clip")
    : [];
  if (prepared.length && !existingNote) {
    try {
      await putAttachments(prepared);
    } catch (cause) {
...
function openWebClipDialog(clip = null, receiveError = "", { preserveTransferState = false, transferProtocol = "" } = {}) {
  ...
  renderWebClipExistingNoteChoice();
  renderWebClipImages();
  if (!webClipDialog.open) webClipDialog.showModal();
  webClipTitle.focus();
}

function selectedWebClipUpdateTarget() {
  const mode = document.querySelector('input[name="webClipSaveMode"]:checked')?.value;
  if (mode !== "update" || !pendingWebClipMatches.length) return null;
  const id = pendingWebClipMatches.length === 1 ? pendingWebClipMatches[0].id : webClipExistingNoteSelect?.value;
  return pendingWebClipMatches.find((note) => note.id === id) || null;
}
...
async function saveWebClip({ textOnly = false } = {}) {
  ...
  if (!urlValidation.ok) {
    webClipReceivedStatus.dataset.outcome = "failure";
    webClipError.textContent = urlValidation.code === "unsupported_scheme"
      ? "このURL形式には対応していません。http:// または https:// のURLを入力してください。"
      : "URLが無効です。入力内容を確認してください。";
    webClipUrl.focus();
    return;
  }
  ...
  try {
    ...
    renderAll();
    openNote(note.id);
    setSaveStatusNotice(clip.clipResult.status === "partial"
      ? `Webクリップを部分保存しました。${clip.clipResult.notice}`
      : "Webクリップを保存しました");
  } catch (error) {
    ...
    console.error("Web clip save failed", {
      stage: "memo_save",
      code: "save_failed",
      imageFailureCount: Array.isArray(error.imageFailures) ? error.imageFailures.length : 0,
      clipResult: clip.clipResult.diagnostic
    });
    webClipReceivedStatus.dataset.outcome = "failure";
    webClipError.textContent = Array.isArray(error.imageFailures)
      ? "選択した画像を保存できませんでした。画像を確認して再試行するか、「画像なしで保存」を選んでください。入力内容は保持されています。"
...
function chartCategoryLabelMarkup(entry, x, y) {
  if (!entry?.visible) return "";
  const label = entry.layout;
  return `<text class="chart-block-label" x="${x}" y="${y}" text-anchor="middle" aria-label="${escapeAttr(label.fullText)}"><title>${escapeHtml(label.fullText)}</title>${label.lines.map((line, index) => `<tspan x="${x}" dy="${index === 0 ? 0 : 14}">${escapeHtml(line)}</tspan>`).join("")}</text>`;
}

function chartUnitMarkup(unit, x, y, maximumWidth) {
  if (!unit) return "";
  const label = chartLabelLayout(unit, { maximumWidth, maximumLines: 1 });
...
function renderHorizontalBarChart(chart, { title, controls, accessibleItems, blockIndex }) {
  ...
  const layout = horizontalBarSegments(chart.items, chart.series, {
    ...
    width,
    mode: chart.appearance.barMode
  });
  const height = Math.max(120, layout.height);
  const axis = chartValueAxisLayout(maximum, { availableSpace: layout.plotWidth, minimumSpacing: Math.max(48, provisionalAxis.labelWidth + 16) });
  const accessible = percentStacked ? chartAccessibleItems(chart, layout) : showStackTotals ? chartAccessibleItems(chart, null, layout.groups) : accessibleItems;
  const bars = layout.groups.map((group) => {
...
function chartPieLabel(segment, unit, mode) {
  if (mode === "none") return "";
  if (mode === "value") return `${chartDisplayNumber(segment.value)}${unit}`;
...
function chartPngControls(blockIndex, enabled) {
  return enabled ? `<div class="chart-png-controls"><button type="button" data-chart-png-index="${blockIndex}">PNG画像として保存</button><button type="button" data-chart-png-copy-index="${blockIndex}">画像をクリップボードへコピー</button><button type="button" data-chart-svg-index="${blockIndex}">SVG画像として保存</button><p class="chart-block-status" role="status" aria-live="polite"></p></div>` : "";
}
...
async function runChartPngFromButton(button) {
  ...
  const svg = button.hasAttribute("data-chart-svg-index");
  const api = window.MemoNexusChartPngExport;
  controls.setAttribute("aria-busy", "true");
  button.setAttribute("aria-busy", "true");
  buttons.forEach(element => element.setAttribute("aria-disabled", "true"));
  status.textContent = svg ? "SVG画像を生成しています…" : copy ? "グラフ画像をコピーしています…" : "PNG画像を生成しています…";
  try {
    const index = Number(svg ? button.dataset.chartSvgIndex : copy ? button.dataset.chartPngCopyIndex : button.dataset.chartPngIndex);
    ...
    if (svg) await api.saveChartSvg(card, block.chart.title);
    else if (copy) await api.copyChartPng(card);
    else await api.saveChartPng(card, block.chart.title);
    status.textContent = svg ? "グラフをSVG画像として保存しました" : copy ? "グラフ画像をクリップボードへコピーしました" : "PNG画像の保存を開始しました";
  } catch (error) {
    status.textContent = svg ? "SVG画像を保存できませんでした。表示中のグラフで再試行するか、『PNG画像として保存』を利用してください。" : copy ? api.chartPngCopyMessage(error)
...
function renderChartDataTable(chart, blockIndex, copyEnabled) {
  ...
  const swatch = (color) => color ? `<span class="chart-data-table-swatch" aria-hidden="true" style="background:${escapeAttr(color)}"></span>` : "";
  const columns = table.columns.map((column) => {
    const detail = [column.kind, column.axis, column.unit ? `単位: ${column.unit}` : ""].filter(Boolean).join("・");
...
function createChartEditor(chartValue, blockIndex, snapshotKey) {
  ...
  chartTypeLabel.textContent = "グラフの種類";
  ...
  [["bar", "棒グラフ"], ["line", "折れ線グラフ"], ["combo", "複合"], ["pie", "円グラフ"]].forEach(([value, label]) => {
    ...
    option.selected = chart.chartType === value;
    chartType.append(option);
  });
  chartTypeLabel.append(chartType);
  fields.append(chartTypeLabel);
  const appearance = document.createElement("div");
  appearance.className = "chart-block-appearance";
  const dataTableLabel = document.createElement("label");
  dataTableLabel.className = "chart-block-appearance-checkbox";
  const dataTable = document.createElement("input");
  dataTable.type = "checkbox";
  dataTable.dataset.chartField = "showDataTable";
  dataTable.checked = chart.appearance.showDataTable;
  dataTable.setAttribute("aria-label", `グラフ${blockIndex + 1}のデータ表を表示`);
  dataTableLabel.append(dataTable, document.createTextNode("データ表を表示"));
  appearance.append(dataTableLabel);
  if (chart.chartType !== "pie") {
    ...
    if (chart.chartType === "bar") {
      ...
      orientationLabel.textContent = "向き";
      const orientation = document.createElement("select");
      orientation.dataset.chartField = "barOrientation";
      orientation.setAttribute("aria-label", `グラフ${blockIndex + 1}の棒の向き`);
      [["vertical", "縦"], ["horizontal", "横"]].forEach(([value, label]) => {
        const option = document.createElement("option");
        option.value = value;
        option.textContent = label;
        option.selected = chart.appearance.barOrientation === value;
        orientation.append(option);
      });
      orientationLabel.append(orientation);
      appearance.append(orientationLabel);
    ...
    labelModeLabel.textContent = "円グラフのラベル";
    const labelMode = document.createElement("select");
    labelMode.dataset.chartField = "pieLabelMode";
    labelMode.setAttribute("aria-label", `グラフ${blockIndex + 1}の円グラフのラベル`);
    [["percentage", "割合"], ["value", "数値"], ["none", "表示しない"]].forEach(([value, label]) => {
      const option = document.createElement("option");
      option.value = value;
      option.textContent = label;
      option.selected = chart.appearance.pieLabelMode === value;
  ...
  const seriesHeading = document.createElement("p");
  seriesHeading.textContent = `系列（最大3件、現在${chart.series.length}件）`;
  seriesPanel.append(seriesHeading);
  chart.series.forEach((series, seriesIndex) => {
    ...
    name.type = "text";
    name.dataset.chartSeriesField = "name";
    name.value = series.name;
    name.setAttribute("aria-label", `系列${seriesIndex + 1}の名前`);
    nameLabel.append(name);
    const colorLabel = document.createElement("label");
    colorLabel.textContent = `系列${seriesIndex + 1}の色`;
    const color = document.createElement("input");
    color.type = "color";
    color.dataset.chartSeriesField = "color";
    color.value = series.color;
    const legacyColorLabel = seriesIndex === 0 && chart.chartType !== "pie"
      ? `グラフ${blockIndex + 1}の${chart.chartType === "line" ? "線" : "棒"}の色`
      : `系列${seriesIndex + 1}の色`;
  ...
  addSeries.dataset.chartAction = "add-series";
  addSeries.textContent = "系列を追加";
  addSeries.disabled = chart.series.length >= 3;
  if (addSeries.disabled) addSeries.setAttribute("aria-describedby", `chart-series-limit-${snapshotKey}`);
  seriesPanel.append(addSeries);
  const seriesLimit = document.createElement("p");
  seriesLimit.className = "chart-block-series-limit";
  seriesLimit.id = `chart-series-limit-${snapshotKey}`;
  seriesLimit.setAttribute("role", "status");
  seriesLimit.textContent = chart.series.length >= 3 ? "系列は最大3つまでです。" : "";
  seriesPanel.append(seriesLimit);
  ...
  chart.items.forEach((item, itemIndex) => {
    ...
    chart.series.forEach((series, seriesIndex) => {
      ...
      value.dataset.chartSeriesValue = "true";
...
function handleChartEditorInput(event) {
  ...
  if (["chartType", "barMode", "barOrientation", "comboAxisMode", "showDataTable"].includes(event.target.dataset.chartField)) {
    const invalidInput = firstInvalidChartNumberInput(editorBlock);
    if (invalidInput) {
      if (event.target.dataset.chartField === "showDataTable") event.target.checked = next.appearance.showDataTable;
      else event.target.value = event.target.dataset.chartField === "chartType" ? next.chartType : next.appearance[event.target.dataset.chartField];
  ...
  } else if (["showDataTable", "showStackTotals", "showValues", "showPoints", "showLegend"].includes(event.target.dataset.chartField)) {
    if (next.appearance[event.target.dataset.chartField] === event.target.checked) return;
    // Reuse the TSV draft boundary; later edits remain deferred until confirmation.
    if (event.target.dataset.chartField === "showDataTable") {
      const snapshot = chartEditorOriginalCharts.get(snapshotKey);
      if (snapshot) snapshot.deferSave = true;
    }
  ...
  } else if (event.target.dataset.chartField === "barOrientation") {
...
function handleChartEditorChange(event) {
  ...
  if (target?.matches?.('input[type="checkbox"][data-chart-field]') && ["showDataTable", "showStackTotals", "showValues", "showPoints", "showLegend"].includes(target.dataset.chartField)) {
...
function handleChartEditorAction(event) {
  ...
  switch (button.dataset.chartAction) {
    ...
    case "add-item":
      ...
      next.series = next.series.map((series) => ({ ...series, values: [...series.values, 0] }));
    ...
    case "add-series":
      if (next.series.length >= 3) return;
      ...
      return;
    case "delete-series": {
      const seriesIndex = Number(button.closest(".chart-block-series-row")?.dataset.chartSeriesIndex);
...
function bindChartDataTooltips(host) {
  ...
  document.addEventListener("focusin", (event) => {
    ...
    } else if (!event.target.closest?.("[data-chart-png-index], [data-chart-png-copy-index], [data-chart-svg-index]")) closeChartTooltip();
  });
  document.addEventListener("focusout", (event) => {
    if (activeChartTooltip?.target === event.target && !event.relatedTarget?.matches?.("[data-chart-datum], [data-chart-png-index], [data-chart-png-copy-index], [data-chart-svg-index]")) closeChartTooltip();
  });
  document.addEventListener("keydown", (event) => {
    chartPointerTarget = null;
...
function bindChartBlockControls() {
  ...
  preview.querySelectorAll("[data-chart-png-index], [data-chart-png-copy-index], [data-chart-svg-index]").forEach((button) => {
...
function localSaveStatusModel() {
  return {
    state: localSaveState.status,
    label: localSaveLabel(localSaveState.status),
    savedAt: localSaveState.lastSuccessAt,
    notice: localSaveState.errorMessage,
    directoryName: localSaveState.directoryName,
    requiresUserAction: localSaveState.requiresUserAction
  };
}
...
function renderSaveStatusPopovers(browser, local) {
  ...
  if (localSavePopoverBody) {
    const description = local.state === "unconfigured" ? "ローカルフォルダ保存はまだ設定されていません。"
      : local.state === "unsupported" ? local.notice
        : local.state === "saved" ? `「${local.directoryName}」へ保存済みです。`
          : local.notice || `「${local.directoryName || "選択フォルダ"}」への保存状態を確認してください。`;
    localSavePopoverBody.innerHTML = statusPopoverMarkup("ローカル保存", local, description, '<button type="button" data-open-local-settings>ローカル保存設定を開く</button>');
  }
  if (combinedSavePopoverBody) combinedSavePopoverBody.innerHTML = `${statusPopoverMarkup("ローカル保存", local, `ローカル: ${local.label}`)}<hr>${statusPopoverMarkup("ブラウザ保存", browser, `ブラウザ: ${browser.label}`)}`;
}
...
function renderSaveStatus() {
  ...
  if (saveStatus) saveStatus.textContent = `ローカル ${local.label}。ブラウザ ${browser.label}${browser.notice ? `。${browser.notice}` : ""}${local.notice ? `。${local.notice}` : ""}`;
  ...
  if (combinedSaveStatusBtn) {
    ...
    const combinedLabel = localNeedsAttention ? local.label : browser.label;
...
function fallbackCopyText(text) {
  ...
  const inputSelection = typeof previousActiveElement?.selectionStart === "number"
    ? [previousActiveElement.selectionStart, previousActiveElement.selectionEnd, previousActiveElement.selectionDirection] : null;
  ...
  try {
    ...
    if (previousActiveElement?.isConnected && typeof previousActiveElement.focus === "function") {
      try {
        ...
        if (inputSelection) previousActiveElement.setSelectionRange(...inputSelection);
      } catch (error) {
...
titleInput.addEventListener("compositionstart", () => {
  titleCompositionNoteId = currentId;
  globalThis.MemoNexusTypingDerivedUiScheduler?.beginComposition(titleCompositionNoteId);
});
...
editor.addEventListener("compositionstart", () => {
  editorCompositionNoteId = currentId;
  globalThis.MemoNexusTypingDerivedUiScheduler?.beginComposition(editorCompositionNoteId);
  editorCaretCompositionActive = true;
  stopEditorCaretAnimation();
});
editor.addEventListener("compositionend", () => {
  const noteId = editorCompositionNoteId;
  editorCompositionNoteId = null;
  globalThis.MemoNexusTypingDerivedUiScheduler?.endComposition(noteId);
...
localSavePopover?.addEventListener("click", (event) => {
  if (!event.target.closest("[data-open-local-settings]")) return;
  setSaveStatusPopoverOpen("", false);
  openSettingsDialog().catch((error) => console.error("Local save settings failed", error));
});
...
if (disconnectLocalFolderBtn) disconnectLocalFolderBtn.addEventListener("click", () => disconnectLocalSaveFolder());
localScanResults?.addEventListener("click", (event) => {
  const button = event.target.closest("[data-local-candidate][data-local-action]");
  if (!button) return;
  applyLocalCandidate(Number(button.dataset.localCandidate), button.dataset.localAction).catch((error) => {
    localFolderStatus.textContent = `処理に失敗しました: ${error.message || error}`;
  });
});
document.addEventListener("keydown", (event) => {
