...
test("本番ローカル保存は固定snapshotとrevision境界を使い、失敗時は保存中を解除する", () => {
  ...
  assert.match(app, /function updateNotesTransaction\(items, \{ markLocalPending = true, preserveStoredCodexThread = false, validateBeforePut = null \} = \{\}\)[\s\S]*if \(markLocalPending\) markLocalWorkspacePending\(\)/);
