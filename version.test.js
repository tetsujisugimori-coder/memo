...
test("全ローカルCSS・JavaScriptを0.5.0のキャッシュ識別子で読み込む", () => {
  ...
  assert.match(html, /style\.css\?v=0\.5\.0-83/);
  ...
  assert.match(html, /geometry-block-utils\.js\?v=0\.5\.0-13/);
  assert.match(html, /geometry-editor-utils\.js\?v=0\.5\.0-13/);
  assert.match(html, /geometry-svg-renderer\.js\?v=0\.5\.0-11/);
  assert.match(html, /geometry-block-editor\.js\?v=0\.5\.0-15/);
