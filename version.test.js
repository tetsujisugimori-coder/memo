...
test("全ローカルCSS・JavaScriptを0.5.0のキャッシュ識別子で読み込む", () => {
  ...
  assert.match(html, /style\.css\?v=0\.5\.0-75/);
  ...
  assert.match(html, /geometry-block-utils\.js\?v=0\.5\.0-3/);
  assert.match(html, /geometry-editor-utils\.js\?v=0\.5\.0-2/);
  assert.match(html, /geometry-block-editor\.js\?v=0\.5\.0-2/);
