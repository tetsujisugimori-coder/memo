...
test("全ローカルCSS・JavaScriptを0.5.0のキャッシュ識別子で読み込む", () => {
  ...
  assert.equal(assetVersions.length, 45);
  ...
  assert.match(html, /style\.css\?v=0\.5\.0-63/);
  assert.match(html, /logo-animation-utils\.js\?v=0\.5\.0-7/);
  assert.match(html, /app\.js\?v=0\.5\.0-125/);
});
