...
* Ubuntuの従来2 runではc10の初回rAF空白が41／90ms、c4は15／14msだったが、上の変更前後ローカル試行では長い空白を再現できなかった。したがって、このローカル結果だけでは該当するUbuntuの空白を「メインスレッド占有」か「WebKit／Playwright待機」へ分類できない。ボタンと直接親の持続的な矩形変化は従来run・今回runで未観測だが、frame間の一時変化や他要素の変化は除外できない。次に価値が高い調査は、同じ診断コードをUbuntu WebKitの手動runで実行し、41〜90msのgapが生じた試行のtimer時系列を照合すること。製品UI、click操作、timeout、待機条件の最適化は行っていない。
