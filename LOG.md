...
* [手動run #398](https://github.com/tetsujisugimori-coder/memo/actions/runs/36189479419) は、対象WebKit計測以外に走るChromium Chart全suiteが `chart-multi-series` 待機で失敗し、run全体は失敗。対象外Chart feature側の失敗で、手動WebKit `chart-combo` jobは成功した。[通常PR run #397](https://github.com/tetsujisugimori-coder/memo/actions/runs/36189380007) は8ジョブすべて成功し、profile入力falseの通常経路を確認。手動run #399でもWebKit `chart-combo` jobは成功し計測を記録したが、Chromium Chart jobを含む他jobの完了を確認中。
