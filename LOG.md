...
* Chart E2Eのモバイル検証では、レイアウトモードの切替完了とコンテキストパネルの通常の閉鎖を待ち、ページを先頭へ戻してからカード表示ボタン中心の`elementFromPoint()`がボタン自身を返すことを確認して実クリックする。固定時間待機、`force`クリック、アプリ状態の直接変更は使わない。
