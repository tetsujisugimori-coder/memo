...
## 2026-08-30 Issue #153 ロゴアニメーション選択カード

* PR #152で完成した生体Nexusを最新版として維持し、`living-nexus`、`typewriter`、`nexus`、`scan`、`off`の5種類を比較できる選択カードへ設定UIを変更した。生体Nexusの9.6秒起動時間、4ノード・4触手、2区間三次ベジェ曲線、触手座標・乱数計画・状態遷移・額縁演出は変更していない。
* カード内デモは左上ロゴの子DOMを`cloneNode(true)`で複製し、同じCSS、`createLogoAnimationController`、`createTentacleGeometry`、再生時間、Reduced Motion判定を利用する。明示的に再生したデモだけのcontrollerを作り、別カード・連続再生・設定終了時は以前のRAFとタイマーを停止する。デモでは通常時のランダムイベントを予約せず、初期表示、`off`、Reduced Motionは完成状態で静止する。
* 設定状態を保存済み・選択中・プレビュー中へ分離した。カード選択は左上へ即時反映するが保存せず、「適用」で初めて`memo-nexus-logo-animation`へ保存する。未適用で閉じた場合は保存値へ戻し、「標準に戻す」は未確定の`living-nexus`選択として扱う。未設定・`daily`・不正値を`living-nexus`へ移行する既存方針を維持した。
* ネイティブradioによる排他的選択、独立した再生button、選択マーク・枠・テキスト、`focus-visible`、`aria-live`を追加した。デスクトップは2列、狭幅は1列とし、デモと説明がカード外へはみ出さない構成にした。
* `node --test`は全880件成功し、ロゴ関連32件とバージョン3件、変更JavaScript 22ファイルの`node --check`、`git diff --check`が成功した。ライト／ダーク、390px、キーボード、連続プレビュー、未適用キャンセル、Reduced Motion、`off`、バックグラウンド復帰の実ブラウザ確認は、検証環境のブラウザサービスを初期化できず未実施とした。
* 配信識別子は`style.css?v=0.5.0-63`、`logo-animation-utils.js?v=0.5.0-7`、`app.js?v=0.5.0-125`へ更新し、変更していない資産の識別子は維持した。

## 2026-08-31 PR #155 レビュー対応：PR #152復元と種類別の代表静止デモ

* PR #155で簡易化されていたロゴ実装を撤回し、PR #152マージ済みの最新`main`を正本として再構成した。9.6秒の生体Nexus、4ノード・4触手、2区間三次ベジェ、位相伝播、先端ノード追従、12〜35秒待機、状態遷移、額縁連動、可視性復帰、乱数境界・循環位相を維持し、`memo-nexus-app-title`と4種類・1.5秒の旧仕様は採用していない。
* 5種類のカードはヘッダーロゴの子DOMを複製し、同じ額縁、核、4触手、4ノード、文字、カーソル、走査光、`createLogoAnimationController`、`createTentacleGeometry`を共有する。カード専用の簡易ロゴは作成していない。
* タイマー不要の代表静止ポーズを追加し、生体Nexusは完成状態、タイプライターは細いカーソル、Nexus接続は触手・ノード・額縁の接続強調、光の走査は中央の走査光、`off`は完成状態と「アニメーションなし」で、設定画面を開いた直後から識別できるようにした。再生時だけ代表ポーズを外し、終了・切替・閉じる操作で停止して代表状態へ戻す。
* 保存済み・選択中・再生中の状態を分離し、カード選択と「標準に戻す」では保存せず、「適用」時だけ既存キー`memo-nexus-logo-animation`へ保存する。未適用で閉じた場合は保存済み設定と通常時の待機へ戻す。未設定・`daily`・不正値を`living-nexus`へ移行する既存方針を維持した。
* `node --test`は全882件成功し、ロゴ関連34件とバージョン3件、変更JavaScriptの`node --check`、`git diff --check`が成功した。実ブラウザ確認は検証環境のブラウザサービスを初期化できず未実施で、5種類の静止差、ライト／ダーク、デスクトップ／390px、連続再生、未適用キャンセル、Reduced Motion、`off`、バックグラウンド復帰を手動確認対象としてPRへ記載する。

## 2026-08-31 PR #155 レビュー対応：ロゴDOM系統の分離

* `living-nexus`と`off`はPR #152の生体Nexus DOMを維持し、`typewriter`、`nexus`、`scan`はbase commit `878ef891e47cdee7be0512d11dfbe905c79f5862`の触手なしDOM、配置、CSS、キーフレームへ復元した。生体系は中央核・ハロー・4触手・4周辺ノード・二重額縁を持ち、従来系は9文字・カーソル・始終点ノード・四辺フレーム・走査光だけを持つ。
* 生体・従来各1個の`template`と、設定値の正規化・`data-logo-family`判定・必要時だけのDOM置換を行う共通関数を追加した。ヘッダーと設定カードは同じ経路を使い、従来3種類で生体NexusのDOMを複製する処理を廃止した。CSSも`living`と`legacy`へスコープし、代表静止デモを各系統の実DOMへ合わせた。
* 系統変更前にcontrollerを`off`へ切り替え、request ID更新、終了・通常待機タイマー解除、初期・開始・motion RAF取消、再生・触手クラス、active属性、プレビュー静止ポーズを解除してから置換する。従来DOMでは触手描画が要素0件として安全に終了し、旧3種類と`off`は通常触手イベントを予約しない。
...
## 2026-08-31 モバイル執筆モード

* 既存のモバイル境界（720px未満）だけで有効になる執筆モードを追加した。本文をユーザーがフォーカスすると開始し、「完了」、別メモへの移動、対象メモの削除、モバイル幅からの離脱で終了する。既存の別ウィンドウ用「↗」は役割を変更していない。
* 執筆中はアプリヘッダー、タイトル入力・タグ・日時・保存詳細・補助情報、関連メモ、NEX-2相当のAIロボット、通常ツールバー、右コンテキストパネルを一時収納し、上部を「完了・省略タイトル・簡易保存状態」、下部を「追加・画像・AI・記法ガイド」の1段に整理した。完了時はコンテキストパネルの元の開閉状態も復元する。
* 「追加」は表・注意書き・解説ブロック・計算、「AI」は要約・選択範囲送信を既存ボタンへ委譲し、画像と記法ガイドも既存処理を再利用した。ツール操作前に既存選択範囲と本文スクロール位置を記録し、操作直前に本文へフォーカス・選択範囲・スクロール位置を戻す。blurだけでは執筆モードを終了しない。
* iPhone Safari向けに`100vh`フォールバックと`100dvh`、上下safe area、flexによる残り高さ配分、本文内部スクロール、キーボード側へ展開する高さ制限付きメニューを使用した。保存共通基盤、保存先切替、ローカル・ブラウザ・Codexスレッドの各保存処理は変更していない。
...
## 2026-09-01 広幅3列レイアウトの境界リサイズ

* 広幅表示の本文―カード間と、中央ワークスペース―右側コンテキストパネル間へ、Pointer Captureによるドラッグ、左右矢印キー、ダブルクリック初期化に対応したアクセシブルな縦セパレーターを追加した。ドラッグ中の反映はCSS変数更新だけを`requestAnimationFrame`へまとめ、文字選択抑止は操作終了・キャンセル・Escape・例外時に解除する。
* 本文320px、カード280px、右側欄240〜520pxを基準に、その時点のbody幅、ワークスペース幅、padding、セパレーター列から変更可能範囲を計算する。幅は専用キー`memo-nexus-layout-widths`へ`editorWidth`と`contextPanelWidth`をJSONでまとめて`localStorage`保存し、不正値を無視して復元時に現在幅へクランプする。右側欄の既定幅は340px、本文の既定幅は従来の`1fr : 0.8fr`相当とした。
* セパレーターは`layoutMode === "wide"`かつ通常ウィンドウだけで有効にし、compact・mobileでは非表示かつフォーカス不能、右側欄を閉じた場合は右境界も非表示とした。既存の1040px／720px境界、カード収納、ドロワー、背景オーバーレイ、`inert`制御、ポップアウト本文単独表示は維持した。
* 配信識別子を`style.css?v=0.5.0-69`、`layout-resize-utils.js?v=0.5.0-1`、`app.js?v=0.5.0-131`へ更新した。`npm test`は全916件成功し、`node --check app.js`、`node --check layout-resize-utils.js`、`git diff --check`も成功した。
* Edge実ブラウザで2本のドラッグ、矢印キー、個別ダブルクリック、再読み込み復元、右側欄の開閉、compact・mobile・wideへの再移行、横スクロール0、ドラッグ終了後の`user-select: auto`復帰を確認し、コンソールエラーは0件だった。本文データを変更しないため、実文字入力は行っていない。

## 2026-09-01 広幅3列リサイズ境界の空白帯・独立保存修正

* 右境界用の10px Grid列が透明な占有幅となり、中央ワークスペースと右側パネルの間に空白帯が見えていたため、`body`をワークスペースと右側パネルの2列へ戻した。右セパレーターは第2列の左端へ`translateX(-50%)`で重ね、約10pxの判定幅を維持しながら、通常線を実境界へ一致させた。wide時は右側パネル自身の左枠を外し、セパレーターの1px線へ一本化した。
* 右側幅の上限計算からセパレーター列10pxの減算を除いた。本文320px、カード280px、右側欄240〜520px、1040px／720pxの境界は維持している。
* 保存時に両方の適用幅を要求値へ書き戻していたことが、未操作側まで固定幅になる原因だった。操作対象の`kind`だけをクランプ済み適用幅で確定する純粋処理へ変更し、`editorWidth: null`を「利用可能幅から既定比率を再計算する」明示状態として保存・復元する。狭いwide表示で一時的にクランプされた未操作側の希望幅は上書きしない。
* 配信識別子を`style.css?v=0.5.0-70`、`layout-resize-utils.js?v=0.5.0-2`、`app.js?v=0.5.0-132`へ更新した。状態遷移、既定比率再計算、オーバーレイ配置、占有幅なし、既存Pointer Capture・Escape・選択抑止・Shift＋矢印・responsive制御の回帰テストを追加し、`npm test`は全920件成功した。
* Edge実ブラウザで、境界の隙間0px、判定幅約10px、通常線約1px、二重線なし、右側240／340／520px、本文・右側の独立変更と再読み込み、本文初期化後の比率再計算、右側開閉、1039px compact・719px mobile・1040／1280／1600px wide、wide復帰時の希望幅復元、横スクロール0を確認した。console error・page errorは0件で、確認後は両幅を既定状態へ戻した。

## 2026-09-01 広幅カード列の未使用領域修正

* 広幅ではJavaScriptが本文列を具体的なpx値へ設定する一方、カード列が単独の`minmax(280px, 0.8fr)`だったため、残り幅の約80%しかカードへ割り当てられず、右paddingとは別に約20%の未使用領域が残っていた。外側のbody Gridは`.workspace`と右側パネルが隙間なく接する2列構成のまま維持した。
* `.workspace`のカード列だけを`minmax(280px, 1fr)`へ変更し、本文列と16pxセパレーターを除く残り幅をすべてカードへ割り当てた。左右padding 16px、本文320px、カード280px、右側欄240〜520px、既定比率計算、独立保存、各リサイズ操作、1040px／720px境界は変更していない。
* Playwrightの`getBoundingClientRect()`と計算済みpaddingを使う`layout-resize.e2e.js`を追加した。1pxの許容差で、外側隙間0px、カード右側16px、本文―カード間16px、残り幅の使い切りを直接検証し、本文最小・既定・中間・最大、右側欄240／340／520px、再読み込み、初期化、開閉、1039px compact、719px mobile、popout、横スクロールなしを確認する。
* 1363pxの修正前は、workspace 1023px、本文541.66px、セパレーター16px、カード346.67px、カード―右側欄102.67px、余計な空白86.67pxだった。修正後はworkspace 1023px、本文541.66px、セパレーター16px、カード433.34px、カード―右側欄16px、余計な空白0pxとなった。1040／1280／1363／1600pxでもカード―右側欄はすべて16pxだった。
* `style.css`の配信識別子を`0.5.0-71`へ更新した。`npm test`は全920件成功し、Playwright座標テストも成功、console error・page errorは0件だった。実ブラウザでも2本の境界ドラッグ、カード追従、右側パネル開閉、compact・mobile・wide往復、二重線なし、横スクロール0を確認した。

## 2026-09-02 Web Clipperの失敗分類・部分保存・取得境界改善

### 変更内容と判断理由

* 現行構成を横断確認し、入口を拡張`popup.js`と本体`?web-clip`、本文入力元を表示済みタブDOM、抽出を`page-extractor.js`、画像取得をService Worker、保存を既存`createNote`／`createWebClipNoteWithImages`と整理した。URLからページHTMLを再取得するサーバー処理は存在しないため、アクセス制限回避やサイト別規則は追加していない。
* `clip-result.js`を追加し、入力検証、ページ取得、HTML解析、メタデータ抽出、本文抽出、画像取得、メモ変換、保存の段階と、`invalid_url`から`unknown`までの分類、利用者向け文言、限定した診断情報、再試行可否、部分保存可否をUI・通信から分離した。
* 通常本文が得られない場合は、利用者の選択文、JSON-LD `articleBody`、Open Graph／通常description、メタデータのみの順で候補を作る。全文方式をリンク方式へ暗黙変更せず、選択文と追記メモを通信・抽出・画像失敗から独立して保持する。
* 画像は既存の20件、1件5MB、合計20MBと個別確定を維持した。Cookie・認証情報・referrerを送らず、HTTP(S)以外、明示的なlocalhost／ループバック／リンクローカル／プライベート宛先、危険なリダイレクト、5回超のリダイレクト、応答サイズ超過を画像単位で拒否する。
* 本体確認画面は成功・部分成功・失敗を意味と色で区別し、画像一部失敗件数を明示する。保存失敗では内部例外を画面へ出さず入力を保持し、同一操作の二重保存をガードした。保存自体は既存の通常メモ／添付トランザクションと同一URL更新選択を再利用した。
* 診断URLは認証情報、query、fragmentを除去し、本文、選択文、追記メモ、Cookie、tokenをログへ出さない。拡張版は`0.3.6`へ更新した。
...
## 2026-09-02 モバイル通常表示のレイアウト回帰修正

* PR #159（merge `941e432`）のモバイル執筆モード自体は現在のmainにも残っており、PR #161以降が削除した回帰ではなかった。通常表示のレスポンシブ規則がすべてCSS Container Queriesへ依存していたため、非対応のiPhone Safariではデスクトップ規則だけが適用される構造だった。また対応ブラウザでも、通常表示は横スクロールするデスクトップツールバー、固定配置の関連メモ・NEX-2、縦に広いタイトル情報を維持し、本文拡大はフォーカス後の執筆モードだけだった。GitHub Pagesはmain直下を配信するlegacy buildで、調査時点のhead `aa8c940`と同じ`style.css?v=0.5.0-72`、`app.js?v=0.5.0-135`を返しており、デプロイ未反映ではなかった。
* 既存の1040px／720px境界を変えず、アプリ全体のContainer Queriesを同値のMedia Queriesへ置き換えた。モバイル通常表示ではヘッダーをロゴ＋主要操作の2段へし、低頻度操作を既存ボタンIDのまま「その他」へ収納した。タイトル・タグ・日時と保存表示を各1段へ圧縮し、関連メモとNEX-2を本文外のsafe-area対応下段へ移した。閉じた右パネルのvisibility優先順位も修正した。デスクトップではdetailsを展開したまま従来の操作列と3列リサイズを維持する。viewportへ`viewport-fit=cover`を追加し、`100dvh`と既存safe-area変数を使用した。
* `mobile-layout.e2e.js`を追加し、390px・430px・820px・狭幅デスクトップ・1280pxを実ブラウザ座標で検証する。横スクロール0、ロゴと表示操作のviewport内配置、本文最低高、本文と関連メモ・NEX-2・保存表示の非重複、補助操作メニュー、右パネル収納、デスクトップ列分離を確認し、390px証跡を`e2e-artifacts/mobile-layout-390.png`へ保存する。修正前は390pxで`linkStatsBtn`の右端428.17pxがviewport 390pxを超えて失敗し、修正後は本文450px、各重複0、console error・page error 0件で成功した。
* 既存`mobile-writing-mode.e2e.js`も320・375・390・430px、wide・compact・メモ切替・ポップアウトの全経路で成功し、執筆時本文711pxと完了後のUI復元を確認した。`node --test`は全938件成功し、配信識別子は`style.css?v=0.5.0-73`、`app.js?v=0.5.0-136`へ更新した。このWindows環境にはPlaywright WebKit実体がないため、Chromiumで自動検証し、iPhone Safari実機のアドレスバー伸縮、ソフトウェアキーボード、ノッチ各形状の最終操作感は未確認として残す。

## 2026-09-02 PR #169 モバイルE2Eの再現可能化とWebKit検証

* `playwright` 1.62.1をdevDependencyと`package-lock.json`へ固定し、`npm ci`だけでNode.js依存関係を復元できるようにした。`test:e2e:mobile:layout`、`test:e2e:mobile:writing`、両方を順に実行する`test:e2e:mobile`を追加し、個人環境の`NODE_PATH`やグローバルパッケージへの依存を除いた。通常のPlaywright管理ブラウザを最初に起動し、既存Windows Chromium実行ファイルはその起動に失敗した場合だけ使う方針を両E2Eで統一した。`node_modules/`はリポジトリへ混入しないよう`.gitignore`へ追加した。
...
* Chart E2Eのモバイル検証では、レイアウトモードの切替完了とコンテキストパネルの通常の閉鎖を待つ。まだ閉じている場合だけ、ページを先頭へ戻してからカード表示ボタン矩形内で`elementFromPoint()`がボタン自身を返す実ヒット領域を探して通常クリックする。ヒット領域がなければ失敗する。開いたまま幅を変える場合は同じカードを閉じずに検証する。カードを開いた後は、CSSスライド移動後のカード矩形がビューポート内へ収まるまで待ってから座標を検査する。固定時間待機、`force`クリック、アプリ状態の直接変更は使わない。
* モバイル幅では、ヘッダーを`z-index: 3`で編集カードとタイトル側操作（`z-index: 2`）より前面に保持し、編集カードを`z-index: 0`の基底スタッキングコンテキストにする。絶対配置のタイトル側操作は`.title-row`を配置基準にして、ビューポート右上へ出ずタイトル行内に収める。これによりエクスポート操作がヘッダーより上へ抜け出さず、カード表示ボタンを実機とE2Eのどちらでも操作できる。系列データ、グラフ描画、保存形式は変更していない。
* CSS配信識別子を`style.css?v=0.5.0-96`へ更新し、既存の固定契約を同期した。修正後にChart E2E（Chromium／WebKit）、Mobile E2E（Chromium／WebKit）、全単体テスト、`git diff --check`を再実行し、push後のGitHub Actionsを確認する。

## 2026-09-12 グラフブロック：積み上げ棒グラフ

* 目的：既存の集合棒グラフへ、項目ごとに最大3系列を下から上へ重ねる積み上げ表示を追加した。100%積み上げ、負数、合計ラベル、横向き表示、外部グラフライブラリは追加していない。
* 保存形式：`chartType`は`bar`、`line`、`pie`のままとし、棒の表示方法を`appearance.barMode: "grouped" | "stacked"`として保存する。値がない旧データと未知・不正な値は`grouped`へ正規化する。棒から折れ線・円へ切り替えても`barMode`を保持し、棒へ戻ると復元する。`schemaVersion: 1`、DB_VERSION、Markdownマーカー、`items`と`series`、旧`items[].value`読込、Undo/Redo、保存共通基盤、Markdown/ZIP経路は変更していない。読むだけでは本文マーカーを書き換えず、編集して確定した場合だけ正規化済み形式を保存する。
* 描画：`stackedBarSegments()`へ座標計算を分離した。全値の正の最大値を`scaleBase`として各値を先に縮小し、縮小値の項目別和と最大和で比率を求めるため、`1e308 + 1e308`のような有限値の加算でもInfinityにしない。同じ項目の系列は同一x・幅を使い、保存済み系列順で基線から連続して描画する。0値は高さ0でrectを作らず、全値0、単一系列、50項目、巨大値でも有限かつ非負のSVG座標にする。
* UIとアクセシビリティ：棒グラフの編集欄だけへ「棒の表示方法」の集合／積み上げ選択を追加し、入力中のフォーカスを失わずプレビューを即時更新する。確定前の切替は既存の取消スナップショットへ含まれ、保存失敗時は取消基準を更新しない。積み上げの数値は十分な高さのあるセグメント中央にだけ表示し、小さい領域では省略する。凡例とスクリーンリーダー用リストは集合棒と同じく全系列の項目名・系列名・値・単位を保持する。
* モバイル：既存のカード内グラフスクロール、凡例折返し、入力表だけの横スクロールを維持した。Chromiumの320px、375px、390px、430pxでdocument/body横方向オーバーフロー0を確認した。Safari/iPhone実機は未確認である。
* テスト：`chart-block-utils.test.js`へbarMode正規化、種類切替での保持、3系列の同一x・連続積み上げ、0値、単一系列、巨大値、元配列不変を追加した。`chart-block.e2e.js`へ集合→積み上げ、色・凡例・全系列読み上げ、値表示、確定、再読込、棒→折れ線→円→棒、取消復元を追加した。キャッシュ識別子は変更した`app.js`、`chart-block-utils.js`、`style.css`だけを更新し、既存の固定契約テストを同期した。
* 検証：`node --check app.js`、`node --check chart-block-utils.js`、`node --check chart-block.e2e.js`、`node --test chart-block-utils.test.js`（22件、fail 0）、`npm test`（1,097件、fail 0）、`npm run test:e2e:chart`（Chromium、成功）、`MEMO_NEXUS_E2E_BROWSER=webkit node chart-block.e2e.js`（WebKit、成功）、`npm run test:e2e:mobile`（Chromium、成功）、`git diff --check`を実行した。PR #219 の GitHub Actions 実行 #34690564182 は、CI checks、Geometry E2E（Chromium）、Chart E2E（Chromium／WebKit）、Mobile E2E（Chromium／WebKit）の全6ジョブが成功した。

## 2026-09-12 グラフブロック：100%積み上げ棒グラフ

* 変更理由と仕様：`appearance.barMode`へ`"percent-stacked"`を追加し、既存の集合・積み上げに加えて「100%積み上げ」を選択できるようにした。項目ごとに表示中の全系列を100%へ正規化し、0%／25%／50%／75%／100%のY軸と、十分な高さの区間内だけの割合ラベルを表示する。割合は整数へ丸め、0%へ丸められる正の小割合だけ小数第1位を使う。描画座標は丸め前の割合を使うため、ラベルの丸め誤差は棒の高さへ影響しない。
* 計算とアクセシビリティ：PR #219の`stackedBarSegments()`を拡張し、既存の全値最大`scaleBase`で各値を先に縮小してから項目内比率を出す。同じ計算経路にしたため、`1e308`級の有限値でもSVG属性、DOM、保存データへNaNやInfinityを出さない。合計0は空の棒にする。100%積み上げの`title`とスクリーンリーダー用リストは項目名、系列名、元の値、計算割合、有限な項目合計を含み、表現不能なほど大きい合計はInfinityを読ませず省略理由を示す。系列色・順序・凡例は従来どおり維持する。
* 保存互換性：割合は表示用レイアウトだけで生成し、`items`や`series`の元の実数値を変更しない。`schemaVersion: 1`、DB_VERSION、Markdownマーカー、既存ブロックID、旧`items[].value`読込、`chartType`フォールバック、未知の`barMode`の`grouped`フォールバックは変更していない。棒→折れ線→円→棒、保存・再読込・再編集、取消では`percent-stacked`、系列、色、凡例、値表示と元値を復元する。
...
## 2026-09-13 グラフブロック：系列並べ替えUI

* 目的と設計：系列設定欄へ各系列の「上へ」「下へ」を追加した。`moveChartSeries()`は正規化済みグラフから新しい`series`配列を返し、系列ID、名前、色、全項目値を同じ系列オブジェクトとして一体で交換する。`items`、項目ID、項目順は変更しない。第1系列の互換性ミラーである`appearance.color`だけは、新しい第1系列の色へ同期する。
* 編集とアクセシビリティ：先頭の上へ、末尾の下へ、1系列時の両操作を無効化する。`chartSeriesAccessibleName()`へ名称規則を集約し、空文字・空白だけは「N件目の系列」へフォールバックする。系列名の入力中は、同じ行の両移動ボタンの`aria-label`だけを即時更新するため、編集画面の再描画、フォーカス喪失、キャレット移動を起こさない。移動後は安定した系列IDで行を再特定して同じ操作（境界時は残る移動操作）へフォーカスを戻し、既存ライブステータスで結果を通知する。不正な数値入力がある場合は移動せず、文字列と最初の不正欄へのフォーカスを既存検証で維持する。
* 表示と保存：系列配列の順番を入力表の見出し・数値列、集合棒、積み上げ、100%積み上げ、折れ線、凡例、スクリーンリーダー用情報、Markdown保存へ共有する。円グラフは並べ替え後の第1系列だけを表示し、残りの系列は保持する。`schemaVersion: 1`、DB_VERSION、Markdownマーカー、グラフ・項目・系列ID、旧`items[].value`読込、最大3系列・50項目、Undo/Redo、自動保存、`flushSave()`、取消スナップショット、Markdown/ZIP経路は変更していない。
* テスト：`chart-block-utils.test.js`へ中央系列の上／下・連続移動、範囲外no-op、不変性、0・小数・巨大有限値、旧形式正規化、直列化、表示系列、通常／100%積み上げ順を追加した。`chart-block.e2e.js`へ1／3系列の無効状態、名前の即時aria同期と空名フォールバック、ID・名前・色・値の一体移動、入力表・集合／積み上げ／100%積み上げ／折れ線／円・凡例への反映、フォーカス、ライブ通知、不正数値保護を追加した。狭幅では系列操作群だけを折り返し、項目表の既存横スクロールを維持する。
* 検証結果：`node --check chart-block-utils.js`、`node --check app.js`、`node --check chart-block.e2e.js`、`node --test chart-block-utils.test.js backup-bundle-utils.test.js version.test.js`（54件、fail 0）、`npm test`（1,106件、fail 0）、`npm run test:e2e:chart`（Chromium、成功）、`MEMO_NEXUS_E2E_BROWSER=webkit npm run test:e2e:chart`（WebKit、成功）、`npm run test:e2e:mobile`（Chromium、成功）、`MEMO_NEXUS_E2E_BROWSER=webkit npm run test:e2e:mobile`（WebKit、成功）、`git diff --check`（成功）。E2Eのpage error／console errorは0件で、320px／375px／390px／430pxの横方向オーバーフローは0である。GitHub Actionsはpush後に確認する。WebKit E2Eの成功はSafari/iPhone実機確認を意味しない。

## 2026-09-13 グラフブロック：通常積み上げ棒の合計値ラベル

* 目的：通常の積み上げ棒で、系列ごとの内訳に加えて項目ごとの総量を比較できるようにした。`appearance.showStackTotals` は既定・欠損・不正値を `false` として正規化し、設定を種類や棒モードの切替、確定、再読込、取消、Markdown/ZIP経路で保持する。`schemaVersion`、DB_VERSION、マーカー、グラフ・項目・系列IDは変更していない。
* 計算と安全性：`chartStackedTotals()` は各項目の全系列値から描画時に合計を導出し、`items`／`series`へ合計を保存しない。0、小数、旧`items[].value`、項目・系列の並べ替え、系列追加削除を不変に扱う。`Number.MAX_VALUE`を超える加算は`Infinity`にせず「上限超過」へフォールバックし、表示・SVG・読み上げへ`NaN`／`Infinity`を出さない。浮動小数点の表示は`formatChartStackTotal()`で短い安全な文字列に整える。
* 描画とUI：棒グラフの表示方法が「積み上げ」の場合だけ、既存フォーム部品で「合計値を表示」を出す。チェックは既存の数値表示と独立し、通常積み上げだけで棒の上部中央へ表示する。合計表示時だけプロット上端を下げてラベル余白を確保し、系列値は棒内、合計は棒外に置く。集合、100%積み上げ、折れ線、円では設定値を保持したまま表示・操作欄を止める。スクリーンリーダーには有効時だけ「項目名、合計値」を追加する。
* テストと検証：`chart-block-utils.test.js`へ設定正規化・直列化・表示対象、全系列合計、0、小数、巨大有限値、上限超過、旧形式、不変性、並べ替え、追加削除を追加した。`chart-block.e2e.js`へチェックの既定値・即時反映・独立切替・SVG内の非重なり・読み上げ・集合／100%積み上げからの復元を追加した。`node --check chart-block-utils.js`、`node --check app.js`、`node --check chart-block.e2e.js`、`node --test chart-block-utils.test.js backup-bundle-utils.test.js version.test.js`（57件、fail 0）、`npm test`（1,109件、fail 0）、`npm run test:e2e:chart`（Chromium、成功）、PowerShell等価のWebKit Chart E2E（成功）、`npm run test:e2e:mobile`（Chromium、成功）、PowerShell等価のWebKit Mobile E2E（成功）、`git diff --check`（成功）を実行した。GitHub Actions run `34781013576` はCI checks、Geometry E2E（Chromium）、Chart E2E（Chromium／WebKit）、Mobile E2E（Chromium／WebKit）の全6ジョブが成功した。E2EのWebKit成功はSafari／iPhone実機確認を意味しない。対象外は横棒、負数、100%積み上げ・集合棒の合計表示、外部ライブラリ、保存基盤の変更である。

## 2026-09-14 PR #227 追加修正：通常積み上げ棒の長い合計値ラベル

* 原因：PR #227の視覚用合計ラベルは合計の有限値をそのまま文字列化していたため、長い整数・小数・大きな指数を隣接項目へ表示すると、項目幅を超えて他の合計ラベルやSVG端へ重なる可能性があった。
...
## 2026-09-18 グラフブロックの負数対応・第1段階

* 基準と作業保護：PR #237をマージ済みの最新main ba0c3ecからfeature/chart-negative-values-phase1を作成。元の変更済み画像、未追跡の手書きHTML・work/を保護し、別worktreeで実装した。開始時の全単体は1,120件、グラフ単体は45件。
* 対応範囲：集合棒（縦／横）・折れ線の入力、描画、保存、再読込を有限な符号付き数値へ拡張。finiteChartNumber()で負の0を0に揃え、項目・系列ID、色、順序、旧単一系列の互換性を維持する。NaN／Infinity／不正文字列・途中入力は入力検証と直列化検証で確定を拒否する。不正な旧保存値の読み込み時の安全化は従来互換として維持する。
* 軸と座標：chartValueRange()は表示中の全系列と0から範囲を求める。chartValueRatio()は最大絶対値で縮小して差を取るため、正負の最大有限値でも差のオーバーフローを避ける。chartBarExtent()で0と値の座標の最小位置・絶対差を使い、縦／横の正負方向と非負のrect寸法を保証する。横棒は0幅の集合棒も安全なrectとして保持する。
* 目盛り・ラベル：既存の数値軸・動的余白を範囲に拡張し、有限な負数目盛りと重複しない0を表示する。極端な桁差で0と重なる小さい側の目盛りだけを間引く。0基準線は既存の同位置軸線を置換し、テーマ変数・aria-hiddenを適用する。値ラベルは符号方向、短縮表示、SVG境界、衝突回避を共通計算し、元値をtitle／ariaに残す。横棒の棒内ラベルには明暗両テーマで読める文字色・縁取りを適用する。空の項目名を除外しても読み上げ値の元インデックスを維持する。
* 未対応形式：通常積み上げ・100%積み上げ・円で全系列のどこかに負数がある場合は、日本語エラーと確定無効化を行う。一時編集を既存スナップショットへ保持して本文への反映・自動保存を止め、集合棒・折れ線へ戻せば全値を復元する。エラー中も取消可能。一時編集中にページを閉じた場合は直前の有効な本文が残る。
...
## 2026-09-18 通常積み上げ棒の発散型・負数対応

* 基準: PR #239の実装・テスト・PR本文を確認し、fetch済みorigin/main（6461c125500ca499192ed062cd617f7f1a7b754f）からfeature/chart-diverging-stacked-barを作成。開始前の画像差分、freehand-canvas.html、work/は変更・コミット対象に含めない。
* 計算: chartDivergingStacks()を追加。各項目の正側・負側を別累計にし、+30/-10/+20は+50/-10として0から独立して積む。相殺すると系列ごとの大きさと方向が分からなくなるため、入力順・ID・色・凡例順を維持した発散型を採用。縦棒・横棒が同じ純粋関数の開始／終了比率を使用する。
* 数値安全性: 全表示項目の最大絶対値で縮小してから累積し、正側最大・負側最小・0から共通軸を導出。PR #239のchartValueRatio()、数値軸、0基準線、有限値正規化を利用。元値への無言クランプは行わず、加算上限は加算前に検出してnull＋overflowで保持し「上限超過」と表示する。目盛りも同じ縮小座標で配置し、表現範囲を超える端は正側／負側の上限超過を明示。極小値の表示丸めで0や同じラベルが重なる場合は0と端の目盛りを優先する。
* 表示: 合計表示オンでは正側端・負側端に独立した合計を表示。片側0は省き、全0は0を一つ表示。正数だけの合計の意味と棒間隔を維持。負側合計の下／左余白を確保し、必要時は短縮表示・棒内値の省略を行い、元の有限値と正負の意味をtitle・aria・読み上げ一覧へ残す。0基準線はテーマ追従・aria-hiddenの既存経路を使用する。
* 入力・保存: 通常積み上げだけ負数拒否を解除。100%積み上げと円は今回対象外で、全系列の負数検査と日本語エラー・確定無効化を維持。既存の一時編集・snapshot・自動保存・flushSave・取消の経路を変更せず、未対応形式から戻ると値・系列情報が復元される。エラー中の取消／再読み込みでも直前の有効本文を保つ。
* 互換性: schemaVersion: 1、DB_VERSION: 6、本文マーカー、items/series/appearance、旧単一系列形式は変更しない。合計や縮小値は保存しない。読んだだけでは本文を書き換えず、取消は旧マーカー文字列も復元する。負の0は0へ正規化し、NaN・Infinity・途中入力は確定を拒否する。配信番号はapp 166、chart utils 19とし、固定参照する契約テストも更新。
* 回帰検証: 単体は正／負／混在／全0／1〜3系列／小数／MIN_VALUE／MAX_VALUE／加算上限／正負の極端な桁差、独立累計、系列ID・色・順序、0寸法、有限座標・目盛り、正負合計、形式往復と保存互換を追加。通常積み上げの負数拒否だけ新契約へ変更し、100%積み上げ・円の拒否は維持。既存テストの削除・skip・弱体化なし。
* E2E: 実入力・保存・再読み込み・再編集・種類切替・エラー中取消／再読み込み・旧本文復元を追加。正／負／混在／全0／上限超過／両方向の桁差を320/375/390/430/1100px、ライト／ダーク、縦／横で確認し、合計ラベル衝突・SVG外文字・正負の接続方向・0重複・非有限属性・ページ横はみ出しを検証する。既存Mobile E2Eの画像はTEMPへ出力して作業前の画像を保護。
...
## 2026-09-18 100%積み上げ棒の発散型・負数対応

* 起点・保護: fetch済みorigin/main 9fb5ed8、PR #239／#241のマージを確認。既存作業ツリーの変更画像・freehand-canvas.html・work/をそのまま残し、別worktreeのfeature/chart-diverging-100-percent-stackedで作業。
* 計算: PR #241のchartDivergingStacks()をpercentオプションで拡張し、縦横の100%積み上げを既存の正負別累積・座標経路へ統合。項目内の正側／負側それぞれの最大絶対値で縮小して合計・割合を求める。+30/-10/+20は+60%/-100%/+40%とし、相殺しない。MAX_VALUE級の複数値や符号間・項目間の極端な桁差も安全に処理。最終端は同じ未丸め累計と分母から±100%に一致させる。
* 軸・表示: 正のみ0〜100%、負のみ-100〜0%、混在-100〜100%。全0は既存の0〜100%軸の0位置で棒・割合ラベルを省略。PR #239／#241の数値目盛り、テーマ追従のaria-hiddenな0基準線、カテゴリ省略と余白、棒内に収まるラベルだけ表示する処理を再利用。負側の割合に符号を付け、title／読み上げ一覧へ項目名・系列名・元値・未丸め割合・側別合計を残す。100%モードに合計ラベルは追加しない。
* 入力・保存: 100%モードの負数拒否のみ解除。円グラフの全系列負数検査、日本語エラー、一時編集保持、確定停止と取消は維持。NaN／±Infinity／空欄／途中入力を拒否し、負の0は0とする。schemaVersion: 1、DB_VERSION: 6、本文マーカー、items/series/appearance、系列ID・名前・色・順序を維持。割合や座標は保存せず、極小値の寸法が0になっても元の有限値を保持する。旧形式は読むだけでは書き換えず、取消時に元マーカーを復元する。
* 配信: app 167、chart utils 20へキャッシュ識別子を更新し、固定参照する既存テストを追従。外部依存・DB変更なし。
* テスト: 旧100%負数拒否の期待値を受入れ・保存へ更新し、円の拒否は維持。単体に1〜3系列、正／負／混在／0／小数／負の0／MIN_VALUE／MAX_VALUE、凍結入力の非変更、正負別最終端、縦横座標、別項目・別符号の桁差、保存・形式切替・非有限入力を追加。E2Eは既存の通常積み上げ検証を100%にも実行し、7データ×5幅（320/375/390/430/1100）×2テーマ×縦横の140通りを追加。±100%目盛りとの端点一致、棒内ラベル・衝突・SVG境界、系列色順、元値と割合の読み上げ、単位%、保存・再読込・再編集・項目並替・取消・円切替・無効入力を検証。
* 制限: 同符号内の極端な桁差では小さい値の描画寸法や割合が浮動小数点精度で0となることがある。元の入力値は保存・読み上げ情報に維持。円の負数、二軸、複合、系列数拡張、CSV/表連携は対象外。Safari／iPhone実機は未確認で、Playwright WebKitと区別する。
* 最終ローカル結果: グラフ単体111/111、npm test 1,186/1,186成功（fail/skip 0）。変更JavaScript 22ファイルのnode --check、git diff --check成功。正式npm run test:e2e:chartはChromium／WebKitとも終了コード0で完走（既存通常積み上げ140通り＋負数100%140通り、page error/console error 0）。main版との正数100%比較200レイアウトは座標・寸法・割合が許容差1e-10以内で一致。元の作業ツリーは開始時の3項目を保持。Mobile／GeometryはPRのCIで確認する。
* 検証時の補正: 既存正数100%の読み上げ期待値へ追加した未丸め割合を反映。新規の端点E2Eは縦目盛りの文字ベースライン補正+4を除いて比較するよう修正。固定待機・タイムアウト延長・検証skip・製品座標の変更は行わない。最終HEADのCI結果はPR本文へ記載する。
* CI追跡: 初回run 35343909433は5/6成功。WebKit Chart E2Eは新規検証前の初期入力でタイトルが「テスト得点点」、単位が空になり待機失敗。グラフ挿入のrequestAnimationFrameによるタイトルへのフォーカス復帰と次の単位入力が競合するため、挿入後のタイトルフォーカス、最初の項目追加後の項目名フォーカスを条件待機し、タイトル・単位の入力直後assertを追加した。製品の挙動、待機時間の上限、既存assertは変更していない。


## 2026-09-18 縦棒・折れ線の単一軸複合グラフ

* 起点・作業保護: PR #239／#241／#243のマージをfetchで確認したorigin/main 92694f0を基準にfeature/chart-combo-single-axisを作成。元作業ツリーの変更画像・freehand-canvas.html・work/を保持し、TEMP配下の別worktreeで実装。
* 目的・設計: 同じ単位の2〜3系列を単一Y軸で比較する縦の集合棒＋1系列の折れ線を追加。chartType: combo、appearance.comboLineSeriesIdに正規化済みの安定IDを保存する。初回・選択系列削除・不正IDは残存する最後の系列IDへフォールバック。名称・色・項目／系列順の変更や種類往復で選択を保持する。
* 共通計算: groupedBarLayout()へ従来の集合棒幅・間隔・0基準の座標計算を移し、通常棒とcomboChartLayout()が共有する。chartValueRange／chartValueRatio／chartBarExtent／lineChartPointsを再利用し、全系列の共通範囲で点をカテゴリ中央へ置く。折れ線マークアップも共通化し、棒の後に線と点を描く。最大有限値同士を直接減算せず、元値を縮小保存しない。
* 表示: 凡例の棒／線スウォッチと日本語表記、タイトル・単位・項目・系列・元値・正負・描画形式の読み上げ情報を追加。既存の軸・0基準線・ラベル折返し・省略・間引きを利用。複合の値ラベルは棒・点・線分・既存ラベルを避け、収まらなければ省略する。元値はtitle／読み上げ情報に維持。
* 保存・入力: schemaVersion: 1、DB_VERSION: 6、本文マーカー、items／series／ID・順序を維持。座標・軸範囲は保存しない。旧形式・不正IDは読込だけでは本文を書き換えない。1系列・円への負数切替は既存snapshotの一時編集経路で本文を保護して確定を止める。取消は元マーカーを復元し、自動保存・flushSave・競合検査を迂回しない。barOrientation／barModeを保持し、複合では選択UIを隠して縦集合棒として描く。棒へ戻すと元設定を復元。
* 配信: app 168、chart utils 21へ更新し、固定参照する全契約テストを追従。外部ライブラリ・CSS・DB変更なし。
* 検証中の修正: 値ラベルの実フォント高が約16.32 SVG単位となる狭幅条件で、従来の14単位の衝突枠では約0.32単位重なった。複合の衝突枠のみ20単位へ拡張し、項目ラベルの領域も確保。E2Eでは高さ0のrectをgetBBoxではなくSVG属性で測定し、折れ線全体の外接矩形ではなく線分単位の領域を検査する。レスポンシブ待機は既存のmobile／wide状態を使用する。固定時間待機・skip・タイムアウト延長・既存assert弱体化なし。
* 制限: 単一Y軸・共通単位・縦集合棒＋1折れ線のみ。二軸、横棒／積み上げとの複合、円との同時表示、1系列、4系列以上、日付軸、CSV／TSV、ズーム／パン、画像出力は対象外。極端な桁差で小さい値の描画寸法が0になる場合も元値は保存・読み上げ情報に保持。Safari／iPhone実機は未確認。
* 実表示検証: 狭幅ではカードをUIで開き、サイドパネルを閉じ、カードの表示座標を条件待機してから測定する。WebKit重点検証で120通りを成功。390pxのライト／ダークのキャプチャもTEMPへ出力した。正式Chart E2Eの最終版は両エンジンで全シナリオを最初から再実行し、成功した。
* 最終ローカル結果: 変更JavaScript 22ファイルのnode --check、グラフ単体132/132、npm test 1,207/1,207成功（fail/skip 0）。正式npm run test:e2e:chartはChromium／WebKitとも終了コード0で完走し、複合120通りと既存通常積み上げ140通り・100%積み上げ140通りを含む全シナリオを成功。npm run test:e2e:mobile（layout＋writing）も両ブラウザで終了コード0。page error／console error 0、非有限SVG属性・負寸法・0線／目盛り重複・ラベル衝突・SVG境界外・意図しないdocument／body横はみ出しなし。git diff --check成功。元ツリーの既存3ファイルのSHA-256一致を確認。CI結果は最終HEADのPR本文へ記載する。


## 2026-09-19 複合グラフの左右2軸表示

### 目的と設計判断

- PR #245のマージコミットbf2c01d9d856c3b8b04229d36d3346a87d4dcd8aをfetch後のorigin/mainで確認し、TEMPの別worktree・feature/chart-combo-dual-axisで実装。元作業ツリーの画像差分、freehand-canvas.html、work/は保持。
- 初期値は単一軸。明示的な左右2軸指定だけで、安定IDのcomboLineSeriesIdが指す1系列を右軸の折れ線、残りを左軸の縦集合棒へ割り当てる。既存の単一軸・棒・折れ線・円の座標計算を維持。
- 純粋関数comboAxisRangesで棒の全値と折れ線の値から独立した範囲を算出。両軸全体に正負がある場合は、それぞれの最大絶対値を±の端点にして0位置を中央へそろえる。片側に値がない範囲は空き領域として拡張する。正数のみは下端、負数のみは上端が共通0。全0の片軸は必要に応じて1単位の空き範囲を使用し、両方全0は下端0。
- 対称範囲を採る理由は、二つの0基準があると棒の高さと折れ線位置を誤読しやすいため。最小限の余白より、明確な共通0とNumber.MAX_VALUEでも有限な端点を優先した。生値の差・和・拡大乗算を行わず、既存chartValueRatioで最大絶対値による縮小後に比率を算出する。入力のクランプ・縮小保存はしない。
- comboChartLayoutは左右範囲、共通zeroRatio・zero、棒座標、折れ線座標を返し検証可能にする。目盛りは長い数値を指数表示に短縮し、元の有限値と単位をtitle・aria-labelへ保持。左右の文字幅から余白を確保し、既存のグラフ内スクロールを維持する。

### UI、保存と後方互換性

- 複合のみ「軸モード」を表示し、左右2軸時だけ「右軸の単位」を表示。左軸は既存unitを利用。左右の単位・軸名、凡例の「棒・左軸」「折れ線・右軸」、尺度が異なる旨の説明を表示。0線は1本で、装飾線は読み上げ対象外。
- appearance.comboAxisMode: single | dual、appearance.comboSecondaryUnitを追加。欠損・不正モードはsingle。右単位は既存のタイトル・単位と同じ改行正規化・trim、DOMテキスト／HTMLエスケープを使用。
- schemaVersion: 1、DB_VERSION: 6、UTF-8 hex JSON本文マーカーを維持。旧データは読むだけでは書き戻さず、既存commitChartBlockChange・scheduleSave・flushSave・保存競合検査・snapshotの編集／確定／取消経路を利用する。
- 軸・種類往復で値、ID、系列順、色、折れ線選択、左右の単位を保持。選択系列削除は残存する最後の系列へ切替。取消は元の本文マーカーを復元する。軸モード変更も不正数値の入力中は停止し、その入力を保持する。
- キャッシュ識別子はapp.js 0.5.0-169、chart-block-utils.js 0.5.0-22へ更新し、すべての固定参照契約テストを追従。

### 検証

- グラフ単体155件、npm test全1,230件成功（既存テストの削除・skip・assert削減なし）。正負、全0、小数、負の0、MIN_VALUE、MAX_VALUE、極端な桁差、独立範囲への包含、0一致、凍結入力、保存と種類往復、単一軸互換性を検証。
- 正式モバイルE2E（layout／writing）はChromium・WebKitとも成功。320／375／390／430px、入力・フォーカス・保存の既存検証を維持し、画像はTEMPへ出力。
- 正式グラフE2EはChromium・WebKitとも終了コード0で成功。各エンジンで二軸160通り（2/3系列×8数値条件×light/dark×320/375/390/430/1100px）、既存単一軸複合120通り、通常積み上げ140通り、100%積み上げ140通りと既存の正負値・編集検証を通過。二軸の0一致、左右目盛り・単位・値ラベルと図形の衝突なし、SVG外へのはみ出しなし、有限属性・非負寸法、ページ横overflowなし、page/console errorなしを実測。保存・再読込・再編集、系列/項目順、折れ線選択と削除、軸/種類往復、不正入力取消、旧マーカー復元も検証。
- 最終コードで変更JavaScript23ファイルの構文検査、npm test全1,230件、git diff --checkを再実行し成功。作業ブランチの差分と元作業ツリーのstatusを確認。
- 初回グラフE2Eは二軸分岐の関数配置ミスで失敗し、縦棒・複合の関数へ限定して修正。その後、負数時の長い単位と値ラベルの衝突を検出し、二軸だけ値ラベルの上端を46へ制限して単位領域を確保した。衝突assertはそのまま維持し、Chromiumの正式グラフE2E全体が成功。固定待機やタイムアウト延長、検証緩和は行っていない。
- Safari／iPhone実機は未確認。Playwright WebKitの結果と実機検証は区別する。

### 対象外と制限

- 複数折れ線、自由な系列別軸割当、横棒／積み上げ／円との複合、3軸、日付軸、ズーム・パン、CSV／表連携、画像／PDF出力は対象外。外部ライブラリ・DB変更なし。
- 左右は異なる尺度なので同じ高さ・傾きが同じ値・変化量を意味しない。各軸内で極端な桁差がある値は浮動小数点／画面解像度により0と重なり得るが、元値は保存・読み上げに保持する。


## 2026-09-19 複合グラフの左右軸タイトルと単位

### 依頼内容と設計判断

- 左右の軸の意味を「売上（万円）」「成長率（%）」のように設定・保存・再表示し、系列ごとのツールチップへ割当軸の単位を反映する依頼。PR #247マージ済み、fetch後のorigin/main f963cf2を確認し、TEMPの別worktree・feat/chart-axis-titles-unitsから作業。元の画像差分・freehand-canvas.html・work/を保護。
- 任意のappearance.leftAxisTitle／rightAxisTitleを追加。左単位はunit、右単位はcomboSecondaryUnitを再利用。新規必須項目・DB移行・外部依存を追加せず、schemaVersion: 1、DB_VERSION: 6、UTF-8 hex JSON本文マーカーを維持。タイトルは既存文字列と同じtrim・改行正規化を行い、欠損時はフィールドを追加しない。
- 対象は複合グラフ（単一軸／左右2軸）。左軸タイトルは常時、右軸タイトルと右単位は二軸時だけ入力可能。複合以外ではタイトル設定と表示を隠すが保持し、種類・軸モード往復で復元する。各入力欄へlabel・aria-labelと例を設定。
- formatChartAxisTitleでタイトル＋単位を合成し、片方だけならその文字列、両方空欄なら従来表示とする。SVGの既存単位位置・一行省略・title・aria-labelを再利用。数値目盛りに単位を繰り返さず、座標・尺度・共通0は変更しない。単一軸のタイトル指定時だけ値ラベルの上端余白を二軸とそろえ、衝突を防ぐ。
- chartSeriesUnitを既存app.jsから純粋関数へ移し、安定IDによる既存割当を共有。SVGのtitleツールチップと読み上げへ元の数値と軸の単位を表示し、数値の文字列保存・換算はしない。
- 左軸に複数の棒系列が入り得るため、軸線・タイトルを特定系列色にはせずテーマに対応した既存の中立色を維持。「左軸・棒」「右軸・折れ線」の文字と凡例で対応を明示。
- 自動保存・flushSave・競合検査・snapshotによる取消経路を維持。MarkdownとZIP用bundle、ローカルMarkdownの実際の書出し／取込関数で本文マーカーと全設定の往復を検証。
- キャッシュ識別子をapp 170、chart utils 23へ更新し全固定参照契約テストを同期。CSS・DB・依存は変更しない。

### 検証

- グラフ単体168/168、全単体テスト1,243/1,243成功（fail/skip 0）。旧マーカー、空欄・片方のみ、保存往復、系列割当、種類切替、座標不変、Markdown／ZIP用bundle／ローカルMarkdown入出力を確認。
- 新規重点E2EはChromiumで成功。実UI作成、メモの別画面への切替と再選択、保存・再読込・再編集、タイトル／単位の空欄、0／負数／小数／最大有限値のツールチップ、二軸解除と種類往復、無効数値入力中の取消を確認。長短タイトル×明暗テーマ×390/1100pxの8条件で既存二軸座標・文字衝突・SVG境界・ページ横overflow検証を再利用。390pxの設定欄と入力操作を確認。
- 変更JavaScript24ファイルのnode --check、git diff --check成功。型チェック・Lintの専用scriptや設定はリポジトリにないため未実行。JavaScript構文検査と既存CIの検証方式を使用する。
- 正式モバイルE2E（layout/writing）はChromium・WebKitとも終了コード0で成功。320/375/390/430px、保存・フォーカス・メモ切替を検証。page/console errorなし。
- 正式WebKitグラフE2Eの初回はアプリ操作前のlocalhostへのpage.goto（domcontentloaded）で30秒タイムアウト。同時実行の影響も切り分けるため同じコマンドを単独再実行し、全シナリオ成功。初回の原因は確定しておらず、再実行では再現しなかった。コード・タイムアウト・検証条件は変更していない。

- 最終ローカル結果: 正式グラフE2EはChromium・WebKitとも終了コード0で完走。各エンジンで既存単一軸120条件・二軸160条件・通常積み上げ140条件・100%積み上げ140条件に加え、新規軸タイトル8条件と編集・保存・ツールチップ検証を成功。page/console errorなし。元作業ツリーの保護対象3ファイルは開始時とSHA-256一致。PRの最終CI結果はPR本文へ記載する。

- CI初回のChromiumグラフE2Eは新規モバイル条件で、locator解決後に再描画で取り外されたfigureを測定しclosestがnullとなった。verifyDualGeometryで現DOMの取得と全座標読取りを一つの同期evaluateにまとめ、判定条件・固定待機・アプリコードは変更せず競合を解消。修正後の軸タイトル重点E2EはChromium・WebKitとも8条件＋保存／再編集／ツールチップ検証を終了コード0で成功。node --checkとgit diff --checkも成功。最終CI結果はPR本文へ記載する。
...
## 2026-09-23 CSV/TSVファイルからの表ブロック読込（Closes #272）

### 目的・設計

- Power Query、Excel、業務システムが出力するCSV/TSVを、グラフへ直接渡さず既存の表ブロックを中継点として取り込む。これにより、既存の表編集、コピー、表→グラフ、PNG/SVG/TSV出力の経路と保存形式を共有する。
- 通常貼り付けでCSVを自動判定すると、カンマを含む文章を表として誤認するため、明示的な`CSV/TSV`ファイル選択だけを入口にした。拡張子だけでCSVのカンマ／TSVのタブを決め、MIMEタイプや内容から推測しない。
- UTF-8（BOM可）だけを`TextDecoder("utf-8", { fatal: true })`で受理する。Shift_JIS、UTF-16、Excel本体、複数ファイル、区切りの推測、数式評価は対象外。5MiB、100行、30列、3000セルを超える場合は部分挿入せず中止する。
- 状態遷移型パーサーで引用符、区切り、CRLF/LF/CR、引用符内改行、`""`、空セルを処理する。既存の貼り付け正規化は末尾空列を削るため、ファイル読込だけは専用検証境界を設け、明示された空列を保存する。
- 既存`tablePasteDialog`をファイルモードで再利用し、ファイル名、形式、行数、列数、セル数、見出し選択だけを表示する。ファイルとArrayBufferは永続化せず、確認後に保持するのは行列と最小限のファイル情報だけである。
- ファイル選択時点のメモID、本文、選択範囲、要求IDを保持し、非同期読込・確認中の変更や古い結果は中止する。確定時だけ既存の表作成、Undo、保存、再描画、左上セルのフォーカス経路を一度ずつ使用する。TABLE_BLOCK_VERSION、UTF-8 hexマーカー、DB_VERSION、既存貼り付けとグラフ変換規則は変更しない。

### 実装・検証経過

- `table-block-utils.js`へCSV/TSVパーサー、空列を保持する検証、5MiB定数を追加し、`app.js`へ共通のデスクトップ／モバイル起点、厳密UTF-8読込、読込状態表示、ダイアログモード切替、競合防止を追加した。READMEに利用方法と制約を追記した。
- `table-file-import.test.js`でBOM、CSV/TSV、改行、引用符、空セル・末尾空列、数式風文字列、エラー、上限、貼り付け自動判定不変を確認した。`table-block.e2e.js`ではPlaywrightの実ファイル選択でCSV/TSV、確認、取消、保存・再読み込み、再編集、Undo/Redo、競合中止を確認し、新しい`e2e-artifacts/table-file-import/`へ確認画像を出力する。
...
## 2026-09-24 Chartツールチップ120条件の計測と改善

* PR #280最終CI（Ubuntu、Node 22、Playwright 1.62.1、run 35881763913）の基準値は、Chromiumが120条件33.7秒／ツールチップ機能49.5秒／Chart E2E全体426.6秒、WebKitが61.4秒／80.9秒／600.5秒。以下のWindowsローカル値とは直接比較しない。
* 専用worktreeのWindows、Node 24.20.0、Playwright 1.62.1で`npm run test:e2e:chart -- --feature chart-tooltips`を同じブラウザー・120条件・1100×820px起点で複数回実行した。環境変数`MEMO_NEXUS_E2E_TOOLTIP_PROFILE=1`の時だけ、12 Chartの準備、24テーマ設定、120条件の画面幅・カード表示待ち、先頭移動、ツールチップ確認、終了操作を1行の構造化ログへ集計する。通常CIではこのログを出さない。
* 変更前の120条件はChromium 28.0／28.0秒、WebKit 72.7／70.3／70.5秒（詳細スクロール診断時70.8秒）。変更後はChromium 25.1／25.0秒、WebKit 60.4／61.5／60.3秒。ツールチップ機能全体もChromium 41.5／41.5秒から39.5／39.2秒、WebKit 100.0／95.6／95.9秒から85.9／88.1／86.0秒となった。通常ログの追加検証でもChromium 24.4秒、WebKit 60.1／59.9秒で120条件が成功したが、Chromiumの別の通常ログ実行は53.5秒と大きく変動したため、ローカルの短縮幅を単一値で断定しない。
* 詳細診断ではWebKitの120条件中46条件で先頭へ実際に横スクロールし、従来のPlaywright `scrollIntoViewIfNeeded()`は計約11.8秒、後続の`focus()`は約0.7秒だった。`focus()`だけではWebKitの375px条件でツールチップが出なかった。先頭フォーカスを保って実UIの`Home`キーを押すと、同じ46条件でスクロールし、120条件すべてのツールチップ・先頭フォーカス・可視性を検査できた。既存の120条件、テーマ／幅、End／Escape、内容・アクセシビリティ・横スクロールassert、保存・再読込などは維持し、先頭可視性assertを追加した。SVG座標とカード座標の差が0.51 CSS pxの条件があり、新assertは1 CSS pxのサブピクセル誤差を許す。
* 変更前WebKitの120条件ではプレビュー表示約28秒（カード位置待ち約14秒、モバイルUI操作約11秒）、先頭移動約12秒、閉じる操作約11秒。320〜430pxは1条件平均約0.54〜0.57秒、1100pxは約0.21秒で、テーマ間・Chart種類間の差は小さい。カードの160ms CSS遷移と実UI開閉は今回維持する。2軸・複合Chartの反復工程、CIの45分上限、productionコードは変更しない。Chart全体の変更後CI値と最終HEADの全ジョブ成否はPR本文で別途確認する。
* 変更前のPR #280の既存Ubuntu CI 2回では、120条件がChromium 37.3／33.7秒、WebKit 62.1／61.4秒。最終成功runの機能全体とChart E2E全体はChromium 49.5／426.6秒、WebKit 80.9／600.5秒だった。変更後コードHEAD `8223860`のCI run 35891733255は8ジョブすべて成功し、120条件がChromium 33.3秒、WebKit 57.1秒、機能全体が50.0／76.2秒、Chart E2E全体が453.7／592.7秒だった。Chromiumは変更前の変動範囲内、WebKitは変更前2回より4.3〜5.0秒短い。変更後CIは1回なので安定した効果とは断定せず、Chart全体の変動をツールチップ変更だけに帰属しない。

## 2026-09-24 2軸Chart E2Eの160描画条件の計測と改善（Issue #283）

* `origin/main`の`16a411f`（PR #282を含む）から専用worktreeを作り、元作業ツリーの画像差分と未追跡ファイルを保持した。Windows、Node 24.20.0、Playwright 1.62.1、1100×820px起点、同一PCでブラウザーごとに逐次実行した。単独コマンドは`npm run test:e2e:chart -- --feature chart-dual-axis`、WebKitは`MEMO_NEXUS_E2E_BROWSER=webkit`を付ける。PowerShellでは`$env:MEMO_NEXUS_E2E_BROWSER='webkit'`を設定する。
* 変更前に、既存の順序「系列数→8データ→ライト／ダーク→5幅」を確認した。`MEMO_NEXUS_E2E_DUAL_AXIS_PROFILE=1`の時だけ、データ入力・プレビュー待ち、テーマ操作、画面幅・レイアウト待ち、モバイルカード表示、図形取得、図形assertion、カード閉操作、スクリーンショットを`performance.now()`で集計し、工程・系列数・データ種別・テーマ・幅の合計と件数を1行のJSONで出す。160条件の個別行は出さない。無効時は時刻サンプルと集計出力を行わない。失敗時は系列数・データ番号・テーマ・幅・工程を例外に付ける。
* 次表はプロファイル有効の逐次実行。単位は秒で、各欄は実行1／実行2。図形欄は`verifyDualGeometry`の取得＋検査。テーマ・データの件数はループ順序により変更前32／16、変更後4／32。160条件と図形検査160回、モバイル開閉128回は同数。

| ブラウザー・状態 | データ＋待ち | テーマ | 幅＋待ち | カード表示 | 図形取得＋検査 | カード閉 | 160条件 | 機能全体 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Chromium 変更前 | 8.6／9.0 | 12.5／13.8 | 8.8／10.2 | 26.7／27.0 | 3.8／4.4 | 12.5／13.9 | 73.4／79.0 | 78.0／84.3 |
| Chromium 変更後 | 19.3／18.4 | 2.3／2.4 | 10.4／9.7 | 28.4／28.1 | 4.7／4.5 | 13.7／13.6 | 79.5／77.4 | 85.5／84.1 |
| WebKit 変更前 | 15.8／17.4 | 55.2／58.4 | 43.8／48.4 | 66.9／70.1 | 6.5／6.6 | 63.8／67.8 | 253.5／270.3 | 270.6／288.2 |
| WebKit 変更後 | 31.5／31.1 | 6.9／7.1 | 43.1／43.0 | 66.1／66.6 | 6.3／6.2 | 62.0／61.1 | 217.5／216.7 | 234.8／234.4 |

* WebKitでは繰り返すテーマ操作が55.2／58.4秒を占め、同じテーマ中に8データを試す順序へ変えると6.9／7.1秒になった。データ入力は約16〜17秒増えたが、160条件は36.0〜53.6秒短縮した。Chromiumの160条件は変更前73.4〜79.0秒、変更後77.4〜79.5秒で重なり、短縮は確認できない。スクリーンショット約0.3〜0.8秒を表から省略したため、工程欄の合計と160条件には小差がある。
* 系列数とデータ種別による系統的な偏りは小さい。変更後WebKitの系列数別合計は2系列98.2／103.2秒、3系列118.4／112.6秒、データ種別別合計は各約24.9〜27.3秒、ライト／ダークは各約108秒。幅別合計は320px約45秒、375px約36〜37秒、390px約39秒、430px約43秒、1100px約14秒で、モバイル幅の実UIカード開閉とレイアウト待ちが残る主な費用。図形の位置・衝突・はみ出し・値・凡例・アクセシビリティのassertionは削らず、図形取得も小さいため変更しない。
...
## 2026-09-24 Ubuntu CIでのChartツールチップ工程別計測

* PR #286を含む`main`の`c0973d7`から隔離ブランチを作る。Actionsの`CI`でこのブランチを選び、手動入力`tooltip_profile=true`と`dual_axis_profile=true`を同時に指定して2回実行する。CLIは`gh workflow run ci.yml --ref ci/tooltip-ubuntu-profile -f tooltip_profile=true -f dual_axis_profile=true`。両入力の既定値は`false`で、trueの手動実行時もChart E2E WebKitステップだけに各環境変数`1`を渡す。Chart Chromium、通常PR／push、falseの手動実行では空値とする。実行コマンド`npm run test:e2e:chart:all`と45分上限は維持する。
* 既存の`[TOOLTIP_PROFILE]`は1行JSONで、`elapsedMs`は長いラベルを使う120条件の準備から終了までを測る。その前の12種類のキーボード・tooltip基本操作と、後の150データ点・保存再読込などは機能全体時間に含まれるが、このJSONには含まれない。`setup`は12行の`[Chart番号, 1100px準備, データ入力, 保存待ち, 描画待ち]`、`themes`は24行の`[Chart番号, テーマ, 変更時間]`。各時間列はmsで、各行の値は丸め済み。
* `samples`は120行で、列0〜2が`[Chart番号, テーマ, 幅]`、列3〜9が`[条件全体, プレビュー全体, フォーカス段階全体, 最初のtooltip確認, Endキー＋最後のtooltip確認, 横スクロール検査, 終了段階全体]`。列10〜13はプレビューの内訳`[幅変更, レイアウト待ち, モバイル操作, カード表示待ち]`、列14〜15はフォーカス段階の内訳`[移動前のスクロール状態取得, focus＋Home＋可視性確認]`、列16〜17はEnd段階の内訳`[Endキー, 最後のtooltip確認]`、列18〜20は終了段階の内訳`[Escapeキー, tooltip消去確認, モバイルカード閉操作]`。`focusScrollChanges`は`[Chart番号, テーマ, 幅, 移動前のスクロール座標4値, 移動後の4値]`、`alreadyFocused`は`[Chart番号, テーマ, 幅]`の診断配列であり、時間列ではない。
* 列3は列4〜9を含み、列4は列10〜13、列5は列14〜15、列7は列16〜17、列9は列18〜20を含むため、親列と内訳列は足さない。丸めと集計処理のごく小さい差もある。2軸JSONの`mobile-card-show`はUI操作＋表示待ちで、tooltipでは`mobileControls`＋`cardWait`に相当する。tooltipの`cardClose`は列9の終了段階に含まれる。2軸の160条件とtooltipの120条件は反復数と検査内容が異なるため、工程の絶対秒数だけで実装の効率を判定しない。
* 同じHEAD `a83d8e3bd5c588b9aa9068b7fa3587549732c17a`で両入力をtrueにした[1回目 run 35936895155](https://github.com/tetsujisugimori-coder/memo/actions/runs/35936895155)と[2回目 run 35937780456](https://github.com/tetsujisugimori-coder/memo/actions/runs/35937780456)はいずれもUbuntu、Node `22.23.2`、`npm ci`で導入した固定Playwright `1.62.1`、全8ジョブ成功。WebKitに両環境変数`1`と各1行JSON、Chart Chromiumに空値とJSONなし。tooltipの`setup`12件、`themes`24件、`samples`120件、2軸JSONの`conditions: 160`、両機能の既存完了メッセージを確認した。下表はWebKitの1回目／2回目、時間は秒。

| 計測区間 | 1回目 | 2回目 |
| --- | ---: | ---: |
| tooltipの120条件（`elapsedMs`、ログ表示は58.6／57.5） | 58.589 | 57.531 |
| tooltip機能全体 | 78.1 | 76.6 |
| 2軸の160条件 | 59.2 | 59.4 |
| 2軸機能全体 | 63.5 | 63.7 |
| Chart全スクリプト（Chart本体＋ファイルexport） | 604.3 | 587.2 |

* tooltipの工程は、親列と内訳列を重ねずに合算した。`tooltip確認`は最初と最後の各120回、`終了段階`はEscape・tooltip消去確認・モバイルカード閉操作を含む。丸めた各行の和と`elapsedMs`には約0.001〜0.020秒の差がある。

| tooltipの工程 | 回数（各run） | 1回目 | 2回目 |
| --- | ---: | ---: | ---: |
| 12 Chartの準備（4列合計） | 12 | 6.312 | 6.383 |
| テーマ変更 | 24 | 6.022 | 5.854 |
| 幅変更＋レイアウト待ち | 120 | 7.532 | 7.243 |
| モバイル操作＋カード表示待ち | 96 | 21.313 | 21.234 |
| フォーカス段階 | 120 | 2.645 | 2.443 |
| 最初＋最後のtooltip確認 | 240 | 7.646 | 7.384 |
| Endキー | 120 | 0.373 | 0.370 |
| 横スクロール検査 | 120 | 0.561 | 0.531 |
| 終了段階 | 120 | 6.184 | 6.069 |

* 内訳は、準備のデータ入力3.398／3.537秒・保存待ち2.822／2.765秒、幅変更6.587／6.293秒・レイアウト待ち0.945／0.950秒、モバイル操作7.128／7.053秒・カード表示待ち14.185／14.181秒、フォーカス段階の移動前状態取得0.756／0.734秒・focus／Home／可視性確認1.884／1.709秒、最初のtooltip確認3.992／3.849秒・最後の確認3.654／3.535秒、終了段階内のカード閉操作5.587／5.487秒。これら内訳と上表の親工程は重ねて足さない。`focusScrollChanges`は両runで46条件、`alreadyFocused`は0条件で、時間には含めない診断件数。
* 同じrunの2軸JSONは以下のとおり。2軸160条件の工程合計にはスクリーンショット2回も含む。tooltipは12 Chart準備＋24テーマ＋120条件を`elapsedMs`に含むが、2軸の160条件はその機能の該当反復だけを表す。両機能の機能全体には、それぞれの計測区間外の検査も含む。

| 2軸の工程 | 回数（各run） | 1回目 | 2回目 |
| --- | ---: | ---: | ---: |
| theme | 4 | 1.247 | 1.212 |
| data-preview | 32 | 11.000 | 10.840 |
| viewport-layout | 160 | 10.236 | 9.832 |
| mobile-card-show | 128 | 28.192 | 29.129 |
| geometry-read | 160 | 1.180 | 1.188 |
| geometry-assert | 160 | 0.015 | 0.015 |
| mobile-card-close | 128 | 6.937 | 6.877 |
| screenshot | 2 | 0.187 | 0.180 |

* tooltipのモバイル操作・表示待ち＋カード閉操作＋幅変更・レイアウト待ちは34.432／33.964秒で120条件の58.8／59.0%。2軸のカード表示・閉操作＋幅変更・レイアウト待ちは45.365／45.838秒で160条件の76.6／77.2%。反復数はtooltipのモバイル96回に対して2軸128回で、区間と検査も異なる。直接の秒数比較や共通処理の速さの断定はしない。tooltipのプレビュー全体は28.846／28.482秒で120条件の49.2／49.5%、その内訳でカード表示待ち14.185／14.181秒が繰り返し最大。次はカード表示待ちと実UI開閉をさらに切り分けて測る価値があるという提案であり、今回その操作や検査は変更していない。
* tooltip機能全体はChart全スクリプトの12.9／13.0%、2軸機能全体は10.5／10.8%。tooltipの120条件は57.531〜58.589秒、機能全体は76.6〜78.1秒、Chart全体は587.2〜604.3秒と変動した。同一run内でも、120条件と160条件の近い総秒数を同じ負荷の証拠とは扱わない。[通常PR run 35936924714](https://github.com/tetsujisugimori-coder/memo/actions/runs/35936924714)は同じHEADで全8ジョブ成功、WebKit／Chromiumとも両環境変数は空値・JSONなし、既存120／160条件も完走した。通常runのWebKitは120条件44.7秒・160条件46.7秒だったが、その差を計測負荷に帰属しない。Windowsの絶対時間とも比較しない。
* [両入力falseの手動run 35938738515](https://github.com/tetsujisugimori-coder/memo/actions/runs/35938738515)も同じHEADで全8ジョブ成功。Chart WebKit／Chromiumの両環境変数は空値、両JSONは0行で、120／160条件と後続の検査は完走した。WebKitは120条件57.3秒・160条件59.6秒だったが、成功した2回のプロファイル計測には混ぜない。今回のCI runに失敗はなく、skip・retry・固定待機・期待値・並列度・CI上限・製品コードは変更していない。

## 2026-09-24 Chartモバイルカード表示の内訳計測（Issue #290）

* PR #288ではtooltipの96回のカード表示待ちがUbuntu WebKitで14.185／14.181秒だった。既存の`tooltip_profile`と`dual_axis_profile`を使い、手動入力trueのChart WebKitだけで内訳を測る。通常PR／push、手動入力false、Chart Chromiumでは詳細JSONを出さない。workflow、Chart実行コマンド、45分上限、製品コード、検査・操作・待機条件は変更しない。
...
## 2026-09-24 Chartモバイルカードのclick前からのページ内観測（Issue #292）

* PR #291の既存列・親工程・120 tooltip条件・160の2軸条件・後続検査を維持する。手動入力`tooltip_profile=true`／`dual_axis_profile=true`が有効なChart WebKitだけで、実際の`#cardPaneBtn`操作の直前にページ内観測を設置する。通常PR／push／手動入力false／Chart Chromiumでは観測を設置しない。workflow、Chart実行コマンド、45分上限、skip、retry、並列度、操作回数、待機条件、期待値、製品コードは変更しない。
* ページ時計`performance.now()`で、ボタン上のcapture段階の実clickイベント、`#previewCard`の`aria-hidden="false"`を`MutationObserver`で最初に確認したコールバック、ARIAが表示状態でカード右端と`innerWidth`の差が1px未満となることを`requestAnimationFrame`で最初に確認したコールバックを記録する。アプリのclickハンドラはARIA属性を同期更新し、モバイルカードはCSSの`transform 160ms ease`で移動するため、属性にはMutationObserver、移動中の幾何にはanimation frameを使う。各時刻は**観測できた時刻**であり、内部処理完了や実際の画面描画の厳密な時刻ではない。
...
* Ubuntu manual run #396 は計測対象より前の `chart-multi-series` で `page.waitForFunction` 30秒timeout（chart-block.e2e.js:1596）となり、今回の変更範囲外で失敗し `chart-combo` に到達せず。これを受け、combo_profile有効時に対象featureだけを動かすworkflow分岐を追加。
* その後、手動workflow #398 と #399 の両方で対象WebKit `chart-combo` featureを単独実行。各回120 geometry条件、保存／再読込までPASS（計測feature全体56.9秒／56.3秒、geometry条件部分45.4秒／48.2秒）。2回のステージ総計はdata-preview 4.18／4.31秒、theme 6.36／6.92秒、viewport-layout 6.45／7.65秒、mobile-card-show 22.21／22.75秒、geometry-read 1.10／1.16秒、geometry-assert 12／13ms、mobile-card-close 4.92／5.26秒、series-setup 90／89ms。最大単一条件はどちらもseriesCount=2/dataset=0/theme=light/width=320（条件内1.27／1.39秒）。値はUbuntu GitHub Actions上の2試行であり、安定した性能特性や他環境への一般化を示さない。
* [手動run #398](https://github.com/tetsujisugimori-coder/memo/actions/runs/36189479419) は、対象WebKit計測以外に走るChromium Chart全suiteが `chart-multi-series` 待機で失敗し、run全体は失敗。対象外Chart feature側の失敗で、手動WebKit `chart-combo` jobは成功した。[通常PR run #397](https://github.com/tetsujisugimori-coder/memo/actions/runs/36189380007) と [手動run #399](https://github.com/tetsujisugimori-coder/memo/actions/runs/36190103797) は全job成功。#397でprofile入力falseの通常経路を、#399でprofile有効時の対象WebKit feature成功とChromiumを含む他matrixの通常suite成功を確認。
* 今回、`mobile-card-show` をcontext状態確認、context閉じクリック／状態待ち、カードクリック、カード表示・右端位置条件待ちへ分ける詳細計測を追加。内訳サンプルは寸法別の既存集計へ混入させず、mobile-card-showの親ステージは従来どおり集計する。カード表示フローや待機・クリック・assertionは変更していない。
* WebKit `chart-combo` の詳細計測実行は120条件PASS。ループは系列数2種×データ6種×テーマ2種×幅5種=120条件で、モバイル幅4種だけがカード表示を実行するため2×6×2×4=96回となる。データ入力／previewは12回、テーマ操作は24回で条件ごとに繰り返す。ローカル今回値は96 calls、total 44,586ms、average 464.4ms、min 253.3ms、max 1,195ms。内訳はcontext状態確認96回/2,561ms（26.7ms平均）、context close click 1回/502ms、close状態待ち1回/43ms、card click 96回/27,035ms（281.6ms平均）、card表示条件wait 96回/14,443ms（150.4ms平均）。丸め前内訳合計と親totalは一致する。これは当該ローカルWebKit試行の値であり、既存GitHub Actionsの2試行（22.21秒／22.75秒）とは環境・試行を分けて扱う。
* 固定sleep／waitForTimeoutは当該ループにない。card表示は`aria-hidden=false`と右端位置の`waitForFunction`で確認し、Playwright locator clickの所要時間にはactionability待ち・イベント処理等を含みうるため、transition時間と同一視できない。CSSはモバイルpreview-cardのtransformに160ms ease transitionを定義する。実際のtransition終了イベントは待たず、表示・位置条件成立を待つ。今回の条件wait平均150.4msはtransition仕様と近いが、遷移時間のみの厳密な分離とは言えない。
* 原因分類はD（複数要因）。UI仕様の160ms transition、96条件で繰り返すカード操作／状態待ち、計測できていないPlaywright呼出し内の処理が混在する。今回のローカル値ではPlaywright card click区間が最大だが、その内訳は未分離。次候補は(1) locator clickのactionability・dispatch・UI eventをさらに観測（削減期待値は未確定、意味への影響小、UI仕様変更なし、難度中）、(2)96回のモバイル準備を条件間で共有できるか検討（上限目安は反復準備時間の総量までだが実現可能な共有分は未測定、テスト意味を変える危険中、UI仕様への影響なし、難度中）。固定待ち削除やtransition短縮の候補は確認されず、今回実装していない。
* 診断有効WebKit `chart-combo` と診断無効WebKit `chart-combo` はどちらも120条件・保存／再読込までPASS（feature所要178.4秒／238.8秒）。環境変動が大きく、この2回だけで診断自体のオーバーヘッド量は結論しない。関連`chart-block-utils.test.js`は217件PASS。`npm test`はrootテストを含む全14,060件中14,016件PASS・44件FAILで、失敗は未追跡の既存`work/`履歴tree内にあり、現在の`index.html`版と過去期待値の不一致。通常`npm test`は`work/`も探索するため既存tree由来の失敗として区別する。
* 次段階として既存`chart-tooltips`計測の96回の`#cardPaneBtn` clickを分解する手動WebKit診断`card_click_internals`を追加。目的は観測のみで、96回、locator、wait、transition、DOM操作、assertionは維持。診断無効の通常CI経路も維持し、専用workflow_dispatch入力が有効な場合はChart tooltip featureだけを両ブラウザで実行し、計測自体はWebKitだけで有効にする。
* 計測境界はNode時計のlocator click直前／Promise resolve、ブラウザcapture listenerのDOM click到達、ボタン・カード・bodyの限定属性を監視するMutationObserverの最初のcallback。Observerはテスト中1回だけ設置し、96件の数値をメモリに貯めて最後に1回回収する。Node／ページ時計は設置時のevaluate往復中点で整列し、その半往復を誤差上限として記録する。区間統計は既存形式に合わせcount・合計・1回平均・最大。大量ログ、毎回evaluate、ファイル書込、layout読取は追加しない。
...
## 2026-09-27 Tooltipカードclick T1→post-app listener境界調査

* main `ded9f1c378d16d91c00569e987415ce3f6e6924e`を確認して同期後、`diag/tooltip-handler-boundary`を作成した。作業開始時から存在する`chart-combo-dual-axis.e2e.js`、`e2e-artifacts/mobile-layout-390.png`、`e2e-artifacts/webkit-inspector-timeline.jsonl`、`freehand-canvas.html`、`svg-review/`、`work/`は保持し、変更していない。
* 現在の`app.js`では`#cardPaneBtn`のアプリclick listenerが、open状態を計算して`setCardPaneOpen(open)`を呼ぶ。これは同期関数で、mobile時は`setContextPanelOpen(false, ...)`等の状態更新を経て`updateResponsiveLayoutUi()`を実行し、body class、preview cardの`aria-hidden`と`inert`、buttonの`aria-expanded`、label等を同期更新する。続けて`focusLayoutPanel(previewCard)`を呼び、同関数が`requestAnimationFrame()`にfocus処理を登録する。したがってlistener return、属性更新、rAF callback、layout/paint完了は別の境界である。
* T1はbutton上のcapture listenerがclick eventを観測した時点であり、アプリlistener開始ではない。`#cardPaneBtn`のアプリlistenerは`app.js`の初期化中に登録され、その初期化完了後に診断を設置できる。対象buttonには該当アプリlistenerが1つで、そこに`stopImmediatePropagation()`／`preventDefault()`はなく、後続のtarget listenerを止めない。app全体の別のevent handlerにある伝播停止処理とは区別する。
* 方式比較：handlerへの直接instrumentationは厳密な同期区間を取れる一方production handler経路を変更するため不採用。capture＋後続target listenerはapp listener登録後にテストから追加でき、eventをcancelせず読み取りのみでT1と同期処理後の境界を同じブラウザ時計で得られるため採用。MutationObserverは最初のcallbackであってhandler return時刻ではなく、補助値に留める。`addEventListener` monkey patchはページ全体への介入になるため不採用。
* 診断実装は既存`chart-tooltip-click-diagnostic.e2e.js`を再利用し、既定OFFのworkflow入力`tooltip_handler_boundary`を追加した。WebKit手動診断時のみT0→T1条件の代表c0/light/320、c4/light/390、c10/light/320でclick直前に一時capture listenerと同一target listenerを追加し、click後すぐ両方removeする。captureと後続listenerはどちらも`performance.now()`を使う。後続listenerはtargetの非capture listenerで、実測`eventPhase=2`（AT_TARGET）。`preventDefault()`、focus変更、DOM変更、event propagation操作はせず、`defaultPrevented`は観測だけ行う。96回全clickへの追加計測はしない。production `app.js`、通常CI、通常E2E、通常benchの経路に変更なし。
* 境界定義：T1=capture listenerでのDOM click観測。T2候補=アプリlistenerより後に登録した同一target非capture listenerの実行開始。値の名称は`domClickCaptureToPostAppListener`。この区間にはT1からアプリlistener実行までのdispatch部分が含まれ得るため、純粋なhandler実行時間や正確なhandler開始→終了とは呼ばない。同期listenerのreturn後に実行されるrAF、layout、paintは含めない。
* WebKit診断ONはtooltip feature全体がPASSし、12 Chart、120条件、150 targets、lifecycle/persistence検査まで完走。選択3回の集計値は合計9ms、平均3.0ms、最大4ms（集計出力のms丸め）。3回すべてで後続listener phaseは2、`defaultPrevented=false`、capture/post-app観測欠落0。これは3回だけの境界観測で、handler所要時間の推定や通常性能値ではない。ON試行のfeature全体339秒は診断実行時間として扱い、性能比較に使わない。
* 同じWebKit tooltip featureを診断OFFでもPASSし、同じ12 Chart、120条件、150 targetsと後続検査が完走した（feature 358.5秒）。実行時間差を診断負荷の因果とは解釈しない。ON/OFFのassertionと操作シーケンスは同じ。
