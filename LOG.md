...
* 追加した辺上配置の回帰条件は、修正前のHEADレンダラーをメモリ上で再現して確認した。等辺印だけの左から右の水平辺で中心Y=7（辺上の期待Y=10）となり失敗し、修正後のテストは成功した。`npm test` は1,058件成功（fail 0）、`npm run test:e2e:geometry` と `npm run test:e2e:mobile` は成功、変更JavaScriptの `node --check` と `git diff --check` は成功した。手動ブラウザー操作、実タッチ端末、Safari、push後CIは未確認。
