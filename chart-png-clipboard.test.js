...
function uiCard(id){const status={textContent:""},attrs=new Map(),buttons=[];const controls={getAttribute:k=>attrs.get(k),setAttribute:(k,v)=>attrs.set(k,v),removeAttribute:k=>attrs.delete(k),querySelector:()=>status,querySelectorAll:()=>buttons};const card={dataset:{chartId:id},isConnected:true};for(const copy of [false,true]){const at=new Map();buttons.push({dataset:copy?{chartPngCopyIndex:id}:{chartPngIndex:id},hasAttribute:()=>copy,getAttribute:k=>at.get(k),setAttribute:(k,v)=>at.set(k,v),removeAttribute:k=>at.delete(k),closest:s=>s===".chart-png-controls"?controls:card});}return {card,controls,buttons,status};}
