...
test("app wiring uses typed requests, table IME holding, and dedicated save-success rendering", () => {
  ...
  assert.match(app, /function openNote\(id\)[\s\S]*?discardNote\(previousId\)/);
  assert.match(app, /moveMemosToTrash[\s\S]*?targets\.forEach\(\(note\) => \{[\s\S]*?cancelNote\(note\.id\)/);
  assert.match(app, /permanentlyDeleteMemos[\s\S]*?targets\.forEach\(\(note\) => \{[\s\S]*?cancelNote\(note\.id\)/);
  assert.match(schedule, /schedule\(note\.id, note\.revision\)/);
  assert.match(schedule, /scheduleAuxiliary\(note\.id, note\.revision\)/);
  assert.match(schedule, /"applyCurrentEditorDraft"/);
  assert.match(schedule, /"scheduleDraftMirror"/);
  assert.match(schedule, /"scheduleSaveTimer"/);
  assert.match(schedule, /"scheduleDerivedUi"/);
  assert.match(schedule, /"updateUndoButton"/);
  assert.match(auxiliary, /"renderPreview", renderPreview/);
  assert.match(auxiliary, /"renderMemoListPanel", renderMemoListPanel/);
  assert.match(auxiliary, /"renderRelated", renderRelated/);
  ...
  assert.match(inputEvents, /titleInput\.addEventListener\("input", handleTitleTypingInput\)/);
  assert.match(inputEvents, /editor\.addEventListener\("input", handleEditorTypingInput\)/);
  ...
  assert.match(app, /completeTypingPerformanceMeasurement\(performanceMeasurement, totalDuration, "auxiliary"\)/);
  assert.match(app, /"renderTableBlockEditors", renderTableBlockEditors/);
  assert.match(app, /shouldMeasureDerived\(noteId, revision, "full"\)/);
  ...
  assert.match(app, /function handleDatabaseVersionChange\(\)[\s\S]*?typingPerformance\.discardTransient\(\)/);
  assert.match(app, /window\.addEventListener\("pagehide"[\s\S]*?TypingDerivedUiScheduler\?\.cancelAll\(\)/);
  assert.match(app, /window\.addEventListener\("pagehide"[\s\S]*?typingPerformance\.discardTransient\(\)/);
