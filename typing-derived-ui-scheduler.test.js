...
test("app wiring uses typed requests, table IME holding, and dedicated save-success rendering", () => {
  ...
  assert.match(schedule, /\$\{prefix\}\.applyCurrentEditorDraft/);
  assert.match(schedule, /\$\{prefix\}\.scheduleDraftMirror/);
  assert.match(schedule, /\$\{prefix\}\.scheduleSaveTimer/);
  assert.match(schedule, /\$\{prefix\}\.scheduleDerivedUi/);
  assert.match(schedule, /\$\{prefix\}\.updateUndoButton/);
  assert.match(auxiliary, /renderPreview\(\)/);
  assert.match(auxiliary, /auxiliary\.derived\.renderMemoListPanel/);
  assert.match(auxiliary, /auxiliary\.derived\.renderPreview/);
  assert.match(auxiliary, /auxiliary\.derived\.renderRelated/);
  assert.match(auxiliary, /auxiliary\.derived\.renderTextStats/);
  assert.match(auxiliary, /auxiliary\.derived\.updateAiTargetPreview/);
  assert.match(auxiliary, /auxiliary\.derived\.total/);
  assert.doesNotMatch(auxiliary, /renderTableBlockEditors/);
  assert.doesNotMatch(tableCommit, /renderPreview\(\)/);
  assert.match(tableCommit, /typingPerformanceContext: performanceContext/);
  ...
  assert.match(app, /table\.input\.total/);
  assert.match(app, /full\.derived\.renderTableBlockEditors/);
  assert.match(app, /full\.derived\.total/);
